...
fn main(){
    ...
    // println!("Hello, world!");
    input![sd: Chars, t: Chars];

    let mut ok_is = vec![];
    for i in 0..sd.len()-t.len()+1{
        let mut ok = true;
        for j in 0..t.len(){
            if sd[i+j] == t[j] || sd[i+j] == '?'{continue;}
            #[cfg(debug_assertions)]
            eprintln!("{i}, {j}, {},{}",sd[i+j], t[j]);
            ok = false; break;
        }
    ...
    let mut res = sd.clone();

    if ok_is.len()==0{
        println!("UNRESTORABLE"); return ;
    }
    let p = *ok_is.last().unwrap();
