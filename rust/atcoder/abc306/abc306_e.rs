...
fn main(){
    ...
    // println!("Hello, world!");
    input![n: usize, k: usize, q: usize, xy: [(Usize1, i64); q]];

    let mut res = 0;
    let mut a = vec![(0, -1); n];

    let mut cands = BinaryHeap::new();
    let mut cur_btrees = BTreeSet::new();
    let mut cur_kmax_idxs = HashSet::new();

    for i in 0..k{
        cur_btrees.insert((0, i, -1));
        cur_kmax_idxs.insert(i);
    }
    for i in k..n{
        cands.push((0, i, -1));
    }
    ...
    for (i, &(x, y)) in xy.iter().enumerate(){
        let &(cur_kmin, kmin_idx, kmin_sedai) = cur_btrees.iter().next().unwrap();
        // #[cfg(debug_assertions)]{eprintln!("{:?}", cur_btrees);}

        let (old_y, old_sedai) = a[x];

        if old_y == y {
            // #[cfg(debug_assertions)]{eprintln!("{:?}", (i, x, y, old_y));}
            #[cfg(debug_assertions)]{eprintln!("{:?}", "0 pass");}
            println!("{}", res);
            continue;
        }
        a[x] = (y, i as i64);
        ...
        if y > old_y{
            if cur_kmax_idxs.contains(&x){
                #[cfg(debug_assertions)]{eprintln!("{:?}", "1 pass");}
                //as is

                assert!(cur_btrees.contains(&(old_y, x, old_sedai)));
                cur_btrees.remove(&(old_y, x, old_sedai));
                cur_btrees.insert((y, x, i as i64));

                res+=y;
                res-=old_y;
            }else{
                if y > cur_kmin{
                    #[cfg(debug_assertions)]{eprintln!("{:?}", "2 pass");}
                    // y into ks.
                    assert!(cur_btrees.contains(&(cur_kmin, kmin_idx, kmin_sedai)));
                    cur_btrees.remove(&(cur_kmin, kmin_idx, kmin_sedai));
                    cur_btrees.insert((y, x, i as i64));

                    res+=y;
                    res-=cur_kmin;

                    cur_kmax_idxs.remove(&kmin_idx);
                    cur_kmax_idxs.insert(x);

                    cands.push((cur_kmin, kmin_idx, kmin_sedai));
                }else{
                    #[cfg(debug_assertions)]{eprintln!("{:?}", "3 pass");}
                    //no eff
                    cands.push((y, x, i as i64));
                }
            }
        }else{
            if cur_kmax_idxs.contains(&x){
                if y < cur_kmin{
                    #[cfg(debug_assertions)]{eprintln!("{:?}", "4 pass");}
                    // old_y out, new big in.
                    let mut new_cand = (0,0, 0);
                    cands.push((y, x, i as i64));

                    loop {
                        let (candy, cand_idx, sedai) = cands.pop().unwrap();
                        if a[cand_idx].0 != candy || a[cand_idx].1 != sedai {continue;}

                        new_cand = (candy, cand_idx, sedai);
                        break;
                    }

                    cur_kmax_idxs.remove(&x);
                    cur_kmax_idxs.insert(new_cand.1);

                    assert!(cur_btrees.contains(&(old_y, x, old_sedai)));
                    cur_btrees.remove(&(old_y, x, old_sedai));
                    cur_btrees.insert(new_cand);

                    res -= old_y;
                    res += new_cand.0;

                }else{
                    #[cfg(debug_assertions)]{eprintln!("{:?}", "5 pass");}
                    //as is
                    assert!(cur_btrees.contains(&(old_y, x, old_sedai)));
                    cur_btrees.remove(&(old_y, x, old_sedai));
                    cur_btrees.insert((y, x, i as i64));
                ...
                #[cfg(debug_assertions)]{eprintln!("{:?}", "6 pass");}
                // noeff
                cands.push((y, x, i as i64));
            }
        }
