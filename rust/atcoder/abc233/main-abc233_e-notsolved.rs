...
fn main(){
    ...
    input![mut x: Chars];
    x.reverse();
    let x = x.iter().map(|x| x.to_digit(10).unwrap() as usize).collect_vec();
    #[cfg(debug_assertions)]{eprintln!("x: {:?}", x);}
    let mut first_sum = x.iter().take(101).sum::<usize>();
    let mut carry = vec![0; x.len()*11];

    let mut res = vec![0; x.len()*11];
    let mut s = first_sum;
    ...
    let mut i = 1;
    while first_sum > 0{
        carry[i] = first_sum % 10;
        first_sum /= 10;
        i+=1;
    }
    #[cfg(debug_assertions)]{eprintln!("carry: {:?}", carry);}

    #[cfg(debug_assertions)]{eprintln!("firstsum: {:?}", s);}
    for i in 1..x.len()*11{
        if i-1< x.len(){
            s -= x[i-1];
        }
        // if i+100 < x.len(){s+=x[i+100];}

        let mut s_with_carry = s+carry[i];

        res[i] = s_with_carry % 10;
        s_with_carry /= 10;
        let mut j = 1;
        while s_with_carry > 0{
            carry[i+j] += s_with_carry % 10;
            s_with_carry /= 10;
            j+=1;
        }
    }
