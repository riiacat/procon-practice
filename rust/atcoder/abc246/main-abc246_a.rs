...
fn main(){
    ...
    let minx = xy.iter().map(|xy| xy.0).min().unwrap();
    let maxx = xy.iter().map(|xy| xy.0).max().unwrap();

    let miny = xy.iter().map(|xy| xy.1).min().unwrap();
    let maxy = xy.iter().map(|xy| xy.1).max().unwrap();

    let points = [(minx, miny), (minx, maxy), (maxx, miny), (maxx, maxy)];

    for p in points.iter(){
        if xy.contains(p){continue;}
        println!("{} {}", p.0, p.1);
        return ;
    }
}
