...
fn main() {
    ...
    let mut poss = vec![vec![0;10]; n];

    for (i, s) in s.iter().enumerate(){
        for (j, s) in s.iter().enumerate(){
            poss[i][s.to_digit(10).unwrap() as usize] = j;
        }
    }

    let mut res = 10000000;
    for target in 0..10{
        let mut poss = poss.iter().map(|ps| ps[target]).collect_vec();
        poss.sort();


        let mut a = 0;
        let mut prev_p = 10000;
        let mut cnts = 0;
        for p in poss.iter(){
            if prev_p != *p{
                cnts = 0;
        ...
        // eprintln!("target, poss, a: {:?}, {:?}, {}", target, poss, a);
