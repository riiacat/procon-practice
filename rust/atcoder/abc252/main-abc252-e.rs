...
fn main(){
    ...
    for (eid, (a, b, c)) in abc.iter().copied().enumerate(){
        adjs[a-1].push((b-1, c, eid));
        adjs[b-1].push((a-1, c, eid));
    }

    let mut pq = BinaryHeap::new();
    let mut added = vec![false; n];

    for (to, d, eid) in adjs[0].iter().copied(){
        pq.push((Reverse(d), to, eid+1));
    }
    added[0] = true;
    ...
    let mut ress = vec![];
    while n_added < n{
        let (Reverse(d), n1, eid) = pq.pop().unwrap();
        // eprintln!("new: {} {} {}", n1, d, eid);
        if added[n1]{continue;}
        added[n1] = true;
        n_added += 1;
        ress.push(eid);

        for (n2, d2, eid) in adjs[n1].iter().copied(){
            if added[n2]{continue;}
            // eprintln!("")
            pq.push((Reverse(d+d2), n2, eid + 1));
        }
    }

    // let mut st
    for eid in ress.iter().copied(){
        print!("{} ", eid);
    }
    println!();
    // println!("{}", s.len() - cnt);
