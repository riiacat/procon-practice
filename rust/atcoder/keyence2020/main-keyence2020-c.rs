...
fn main(){
    ...
    for i in 0..n{
        if i < k{
            ...
            print!("{} ", if s<1_000_000_000 {1_000_000_000}else{1_000_000_000-1});
            v.push(if s<1_000_000_000 {1_000_000_000}else{1_000_000_000-1});
        }
    }
    println!();

    // let mut cum = vec![0];
    // cum.extend(v.iter().cumsum::<usize>());
    // // eprintln!("{:?}", cum);

    // let mut cnt = 0;
    // for i in 0..n+1{
    //     for j in i+1..n+1{
    //         if cum[j] - cum[i] == s{cnt+=1;}
    //     }
    // }
    // eprintln!("naive check: {}", cnt);
