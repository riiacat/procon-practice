...
fn main(){
    ...
    // println!("Hello, world!");
    // input![t:usize, n: [usize; t]];
    // input![n:usize, a: [usize; n]];
    input![n:usize, m:usize, p: [Usize1; n-1], xy: [(Usize1, i64); m]];
    ...
    #[cfg(debug_assertions)]{eprintln!("{:?}", childs);}
    ...
    for (x, y) in xy{
        if ys[x].is_none(){
            ...
            ys[x] = Some(max!(y, ys[x].unwrap()));
        }
    }

    let mut res = vec![false; n];

    tree_dfs(0, &childs, &mut res, &ys, 0);

    for (i, b) in res.iter().enumerate(){
        #[cfg(debug_assertions)]{eprintln!("{:?}", (i, b));}
    }
...
fn tree_dfs(cur: usize, childs: &Vec<Vec<usize>>, res: &mut Vec<bool>,  ys: &Vec<Option<i64>>, cur_max: i64){
    #[cfg(debug_assertions)]{eprintln!("{:?}", (cur, cur_max));}

    if cur_max > 0 || ys[cur].is_some(){
        res[cur] = true;
    }
    let mut cur_max = cur_max-1;

    if let Some(y) = ys[cur]{
        cur_max = max(cur_max, y);
    }

    for &child in childs[cur].iter(){
        tree_dfs(child, childs, res, ys, cur_max);
    }
}
