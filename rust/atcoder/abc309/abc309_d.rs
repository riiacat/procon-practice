...
fn main(){
    ...
    input![n1:usize, n2:usize,m:usize, ab: [(Usize1, Usize1); m]];
    let n = n1+n2;

    // let mut adjs = vec![vec![]; n1+n2];
    ...
    for (a, b) in ab{
        if a < n1{
            //add a, b to g1
            g1.add_edge(nodes1[a], nodes1[b], ());
        }else{
            //add a, b to g2
            g2.add_edge(nodes2[a-n1], nodes2[b-n1], ());
        }
        // adjs[a].push(b);
        // adjs[b].push(a);
    }

    // do dijkstra to g1, g2
    let d1 = dijkstra(&g1, nodes1[0], None, |_| 1_usize);
    let d2 = dijkstra(&g2, nodes2[n2-1], None, |_| 1_usize);

    #[cfg(debug_assertions)]{eprintln!("d1: {:?}", d1);}
    #[cfg(debug_assertions)]{eprintln!("d2: {:?}", d2);}

    let max1 = d1.values().map(|d| *d).max().unwrap();
    let max2 = d2.values().map(|d| *d).max().unwrap();
