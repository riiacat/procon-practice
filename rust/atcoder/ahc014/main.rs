...
// ##########
lazy_static! {
    static ref N: Mutex<usize> = Mutex::default();
    static ref M: Mutex<usize> = Mutex::default();
    static ref XY: Mutex<Vec<(i64, i64)>> = Mutex::default();
    static ref XY_SET: Mutex<HashSet<(i64, i64)>> = Mutex::default();
}
...
struct State {
    ...
    // state
    added_xy: Vec<(usize, usize)>,
    cant_put_point: Vec<(usize, usize)>,
    cant_put_edge: Vec<(usize, usize, [bool; 8])>,
    current_score: i64,
}

// impl Struct {
//     fn validate_new_point()
// }
...
fn main() {
    ...
    {
        let mut n_val = N.lock().unwrap();
        *n_val = n;

        let mut m_static = M.lock().unwrap();
        *m_static = m;

        let mut xy_static = XY.lock().unwrap();
        *xy_static = xy;

        let mut xyset_static = XY_SET.lock().unwrap();
        *xyset_static = HashSet::from_iter(xy_static.iter().cloned());
    }

    println!("{:?}", N.lock().unwrap());
    println!("{:?}", M.lock().unwrap());
    println!("{:?}", XY.lock().unwrap());
