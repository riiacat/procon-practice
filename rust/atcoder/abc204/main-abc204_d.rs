...
// #[fastout]
fn main(){
    /* KEY 実質１つのオーブンしか考えなくていいことに気づく。制約からナップザックぽい感じに気づく。
     */
    // let mut stdin = LineSource::new(BufReader::new(io::stdin()));
    // macro_rules! input(($($tt:tt)*) => (proconio::input!(from &mut stdin, $($tt)*)));

    // println!("Hello, world!");
    input![n: usize,mut t: [usize; n]];

    t.sort();
    #[cfg(debug_assertions)]{eprintln!("{:?}", t);}
    ...
    for i in 0..n{
        #[cfg(debug_assertions)]{eprintln!("{:?}", dp[i].iter().take(20).collect_vec());}
        for j in 0..100001{
            if j + t[i] < 100001{
                dp[i+1][j+t[i]] |= dp[i][j];
            }
    ...
    for j in 0..100001{
        if (j as f64 ) < (s as f64 /2.0) {continue;}
        if dp[n][j]{
            println!("{}", j);
            return ;
        }
