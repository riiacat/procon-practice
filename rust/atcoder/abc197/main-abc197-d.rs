...
fn main() {
    input![n:f64];
    input![x: f64, y: f64, x2:f64, y2: f64];

    let origin = ((x+x2)/2.0, (y+y2)/2.0);
    let theta = 2.0*PI/n;

    eprintln!("origin={:?}, theta={}", origin, theta);

    let x = x-origin.0;
    let y = y-origin.1;
    // let x2 = x2-origin.0;
    // let y2 = y2-origin.1;
    eprintln!("moved x={}, y={}", x, y);

    let rotx = theta.cos() * x - theta.sin()*y;
    let roty = theta.sin() * x + theta.cos()*y;

    eprintln!("rotated x={}, y={}", rotx, roty);


    println!("{} {}", rotx+origin.0, roty+origin.1);
}
