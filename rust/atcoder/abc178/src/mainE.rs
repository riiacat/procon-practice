...
fn main() {
    ...
    let sum_d1 = xy.iter().map(|p| (p.0 + p.1) as i64).collect_vec();
    let sum_d2 = xy
        .iter()
    ...
    let sum_d3 = xy
        ...
        .map(|p| (-(p.0 as i64) - (p.1 as i64)))
        .collect_vec();
    let sum_d4 = xy.iter().map(|p| (p.1 as i64 - (p.1 as i64))).collect_vec();

    let ans = max!(
        abs(sum_d1.iter().min().unwrap() - sum_d1.iter().max().unwrap()),
        abs(sum_d2.iter().min().unwrap() - sum_d2.iter().max().unwrap()),
        abs(sum_d3.iter().min().unwrap() - sum_d3.iter().max().unwrap()),
        abs(sum_d4.iter().min().unwrap() - sum_d4.iter().max().unwrap()),
    );
