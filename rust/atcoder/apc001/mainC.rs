...
fn main() {
    ...
    println!("{}", 0); io::stdout().flush().unwrap();
    let s = get_line();
    ...
    while abs(ng - ok) > 1 {
        ...
        //eprintln!("ng: {}, mid: {}, ok: {}", ng, mid, ok);
        println!("{}", mid); io::stdout().flush().unwrap();
        let s = get_line();
    ...
    println!("{}", ok);io::stdout().flush().unwrap();
    let s = get_line();

    if s != "Vacant" {
        println!("{}", n -1);io::stdout().flush().unwrap();
        let s = get_line();
    }
}
