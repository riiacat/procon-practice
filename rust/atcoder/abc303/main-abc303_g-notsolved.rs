...
fn main(){
    ...
    // println!("Hello, world!");
    // input![n: usize, h: i64, td: [(i64, i64); n]];
    input![n: usize, a: i64,  b: usize,  c: i64,  d: usize, x: [i64; n]];

    //dp[turn][l][r]
    let mut dp = vec![vec![vec![0_i64; n+1]; n+1]; 2];

    let mut xcum = x.iter().cumsum::<i64>().collect_vec();

    for i in 0..=n{
        for j in 0..=n{
            dp[0][i][j] = -1_000_000_000_000_000;
            dp[1][i][j] = 1_000_000_000_000_000;
        }
    }
    ...
    #[cfg(debug_assertions)]{eprintln!("{:?}", xcum);}

    for d in 1..=n{
        for l in 0..n{
            let r = l+d; if r > n{break;}

            //satan or utan
            //TODO r-1かも
            chmax!(dp[0][l][r], dp[1][l+1][r] + x[l]);
            chmax!(dp[0][l][r], dp[1][l][r-1] + x[r-1]);

            chmin!(dp[1][l][r], dp[0][l+1][r] - x[l]);
            chmin!(dp[1][l][r], dp[0][l][r-1] - x[r-1]);

            // a en
            let kosu = min!(b, r-l);
            // #[cfg(debug_assertions)]{eprintln!("lkosu: {:?}", (l, kosu));}
            chmax!(dp[0][l][r], dp[1][l+kosu][r] + xcum[l+kosu-1]- if l == 0{0}else{xcum[l-1]} - a);
            #[cfg(debug_assertions)]{eprintln!("(l, r, kosu, r-kosu),: {:?}", (l, r, kosu, r-kosu), );}
            chmax!(dp[0][l][r], dp[1][l][r-kosu] + xcum[r-1]- if r-kosu == 0{0}else{xcum[r-kosu-1]} - a);

            chmin!(dp[1][l][r], dp[0][l+kosu][r] - (xcum[l+kosu-1]- if l == 0{0}else{xcum[l-1]} - a));
            chmin!(dp[1][l][r], dp[0][l][r-kosu] - (xcum[r-1]- if r-kosu == 0{0}else{xcum[r-kosu-1]} - a));

            // c en
            let kosu = min!(d, r-l);
            chmax!(dp[0][l][r], dp[1][l+kosu][r] + xcum[l+kosu-1]- if l == 0{0}else{xcum[l-1]} - c);
            #[cfg(debug_assertions)]{eprintln!("2: (l, r, kosu, r-kosu),: {:?}", (l, r, kosu, r-kosu), );}
            chmax!(dp[0][l][r], dp[1][l][r-kosu] + xcum[r-1]- if r-kosu == 0{0}else{xcum[r-kosu-1]} - c);

            chmin!(dp[1][l][r], dp[0][l+kosu][r] - (xcum[l+kosu-1]- if l == 0{0}else{xcum[l-1]} - c));
            chmin!(dp[1][l][r], dp[0][l][r-kosu] - (xcum[r-1]- if r-kosu == 0{0}else{xcum[r-kosu-1]} - c));

            #[cfg(debug_assertions)]{eprintln!("dp[0][{l}][{r}]: {:?}", dp[0][l][r]);}
            #[cfg(debug_assertions)]{eprintln!("dp[1][{l}][{r}]: {:?}", dp[1][l][r]);}
        }
    }
