...
fn main(){
    ...
    let mut adjs = vec![HashSet::new(); n];
    for &(u,v) in uv.iter(){
        adjs[u-1].insert(v-1);
        adjs[v-1].insert(u-1);
    }

    let mut jisu = vec![0; n];
    for i in 0..n{jisu[i] = adjs[i].len()}
    let mut cand = VecDeque::new();
    let mut res = vec![];

    for i  in 0..n{
        if jisu[i] == 1{cand.push_back(i)}
    }

    let mut determined = vec![false; n];

    while !cand.is_empty() {
        let nid = cand.pop_front().unwrap();
        if determined[nid]{continue;}
        #[cfg(debug_assertions)]{eprintln!("newedge: {:?}", nid+1);}
        let center = *adjs[nid].iter().next().unwrap();
        determined[nid] = true;

        res.push(adjs[center].len());

        let froms =adjs[center].iter().copied().collect_vec();
        for &from in froms.iter(){
            determined[from] = true;
            let tos = adjs[from].iter().copied().collect_vec();
            for &to in tos.iter(){
                if to == center{continue;}
                adjs[to].remove(&from);
                jisu[to]-=1;
                if jisu[to]==1{cand.push_back(to)}
            }
        }
    }

    // println!("{}", min!(dp[0][s.len()], dp[1][s.len()]));
    res.sort();
    for v in res.iter(){
        print!("{} ", v);
    }
