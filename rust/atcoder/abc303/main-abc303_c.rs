...
fn main(){
    ...
    // println!("Hello, world!");
    input![n:usize, m:usize, h:i64, k:i64, S: Chars, xy: [(i64, i64); m]];

    let mut cur = (0, 0);

    let items: HashSet<(i64, i64)> = HashSet::from_iter(xy.iter().copied());
    ...
    for i in 0..n{
        ...
        #[cfg(debug_assertions)]{eprintln!("{:?}", next);}
        ...
        if items.contains(&next) && !used.contains(&next) && h < k{
            used.insert(next);
            h=k;
        }
        cur = next;
