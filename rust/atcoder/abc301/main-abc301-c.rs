...
fn main(){
    ...
    for &s in s.iter(){
        cnt0.entry(s).and_modify(|e| *e += 1).or_insert(1_i64);
    }
    // for &t in t.iter(){
    //     cnt1.entry(t).and_modify(|e| *e += 1).or_insert(1_i64);
    // }

    // let cnt_at0 = *cnt0.get(&'@').unwrap_or(&0);
    // let cnt_at1 = *cnt1.get(&'@').unwrap_or(&0);

    for &t in t.iter(){
        // eprintln!("t: {}", t);
        if t == '@'{continue;}

        let c = *cnt0.get(&t).unwrap_or(&0);
        if c > 0{
            cnt0.insert(t, c-1);
        }else{
    ...
    let mut cnt_at1 = t.iter().filter(|&&c| c == '@').count() as i64;
    for (&s, &c) in cnt0.iter(){
        if c == 0 {continue;}
