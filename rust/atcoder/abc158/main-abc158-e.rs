...
fn main(){
    ...
    input![n: usize, p: usize, s: Chars];
    let s = s.iter().map(|s| s.to_digit(10).unwrap() as usize).collect_vec();

    // let mut st = SegTree::new(1_000_000);

    // let mut mods = vec![vec![0; 200005] ;10];
    ...
    // for a in 0..10{
    //     for d in 1..200005{
    //         mods[a][d] = (mods[a][d-1] * 10) % p;
    //     }
    // }

    if p == 2 || p==5{
        let mut cnts = 0_usize;
        for (i, s) in s.iter().copied().enumerate(){
            if s % p == 0{
                cnts += i+1;
            }
        }
    ...
    let mut mdcnts = vec![0_usize; p+1];
    // mdcnts[0] += 1;
    let mut res = 0_usize;
    let mut oldmd = 0;
    for (i, s) in s.iter().rev().copied().enumerate(){
        let md = ((mod10*s) + oldmd)%p;
        res += mdcnts[md];
        mdcnts[md] += 1;
        if md==0{
            res+=1;
        }
        // println!("i,s,mdcnts,res,mod10,md: {},{},{:?},{},{},{}", i,s,mdcnts,res,mod10,md);
