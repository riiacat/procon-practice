...
fn main(){
    ...
    input![n: usize, ab: [(usize, usize); n-1], mut c: [usize; n]];
    c.sort_by_key(|c| Reverse(*c));
    ...
    cs[a-1]=Some(c[0]); cs[b-1]=Some(c[1]);

    for i in 2..n{
        for (eid, (a, b)) in ab.iter().copied().enumerate(){
            if used[eid]{continue;}
            if !nodes.contains(&a) && !nodes.contains(&b){continue;}
            // eprintln!("eid: {}", eid);
            used[eid] = true;
            nodes.insert(a); nodes.insert(b);
            if cs[a-1].is_some(){cs[b-1]=Some(c[i])}
            else{cs[a-1] = Some(c[i])}
            break;
        }
    }
    let mut res = 0_usize;
    for (a, b) in ab.iter().copied(){
        res += min!(cs[a-1].unwrap(), cs[b-1].unwrap())
    }
    println!("{}", res);
