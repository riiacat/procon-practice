...
fn main(){
    ...
    // println!("Hello, world!");
    input![n: usize, q: usize, ab: [(usize, usize); n], cd: [(usize, usize); q]];

    let mut cur_en = vec![0; n];
    let mut en_trees = vec![BTreeSet::new(); 200000];
    for (i, &(a, b)) in ab.iter().enumerate(){
        cur_en[i] = b-1;
        en_trees[b-1].insert((a, i));
    }

    let mut maxes = BTreeSet::new();
    let mut max_memo = vec![0; 200000];
    let mut exist_strongest = vec![false; 200000];

    for (i, en_tree) in en_trees.iter_mut().enumerate(){
        if let Some((rate, id)) = en_tree.iter().next_back().copied(){
            exist_strongest[i] = true;
            maxes.insert((rate, id, i));
            max_memo[i] = rate;
        }
    }

    for &(id, new_en) in cd.iter(){
        let id=id-1;let new_en=new_en-1;

        let rate = ab[id].0;
        let old_en = cur_en[id];

        let old_en_max = *en_trees[old_en].iter().next_back().unwrap();
        en_trees[old_en].remove(&(rate, id));
        let new_en_max = en_trees[old_en].iter().next_back();

        if old_en_max == (rate, id){
            maxes.remove(&(rate, id, old_en));
            if new_en_max.is_some(){
                let new_en_max = new_en_max.unwrap();
                maxes.insert((new_en_max.0, new_en_max.1, old_en));
            }
        }

        let new_en_max = en_trees[new_en].iter().next_back();
        if new_en_max.is_none() || rate > new_en_max.unwrap().0 {
            if let Some(new_en_max) = new_en_max{
                maxes.remove(&(new_en_max.0, new_en_max.1, new_en));
            }
            maxes.insert((rate, id, new_en));
        }
        en_trees[new_en].insert((rate, id));
        cur_en[id] = new_en;

        println!("{}", maxes.iter().next().unwrap().0);
    }
