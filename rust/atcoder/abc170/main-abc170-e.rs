...
fn main(){
    /* KEY
    BtreeSetをうまく使う。データ構造を二重もつ（各幼稚園、最大値の集合）、multiset的に使うBtree
     */
    ...
    // input![q:usize, st: [(usize, usize); q]];
    input![n:usize, q:usize, ab: [(usize, usize); n], cd: [(usize, usize); q]];

    let mut rates = vec![];
    let mut zoku = vec![];

    let mut r_st = vec![BTreeSet::new(); 2*100000+1];
    let mut best_st = BTreeSet::new();
    // let mut r_cnts = vec![HashMap::new(); 2*100000+1];
    // let mut pq =vec![BinaryHeap::new(); 2*100000+1];
    ...
    for (i, (a, b)) in ab.iter().copied().enumerate(){
        ...
        r_st[b].insert((a, i+1));
        // r_cnts[b].entry(a).and_modify(|a| *a+=1).or_insert(1_usize);
        // pq[b].push(Reverse(a));
    }

    for i in 0..200000+1{
        if !r_st[i].is_empty(){
            best_st.insert(*r_st[i].iter().next_back().unwrap());
        }
    }

    // eprintln!("bestst: {:?}", best_st);

    for (c, d) in cd.iter().copied(){

        let rate = rates[c-1];
        let old_en = zoku[c-1];
        // eprintln!("c: {}, rate: {}, old_end: {}", c, rate, old_en);

        let old_bst = *r_st[old_en].iter().next_back().unwrap();
        // eprintln!("oldbst: {:?}", old_bst);
        if old_bst.1 == c{
            best_st.remove(&old_bst);
            while !r_st[old_en].is_empty(){
                let old_bst = *r_st[old_en].iter().next_back().unwrap();
                // eprintln!("remove? oldbst: {:?}", old_bst);
                if old_bst.1 != c && zoku[old_bst.1-1] == old_en{break;}

                r_st[old_en].remove(&old_bst);
            }
            if !r_st[old_en].is_empty(){
                best_st.insert(*r_st[old_en].iter().next_back().unwrap());
            }
        }

        if let Some(old_bst) = r_st[d].iter().next_back(){
            best_st.remove(old_bst);
        }
        r_st[d].insert((rate, c));
        best_st.insert(*r_st[d].iter().next_back().unwrap());

        println!("{}", best_st.iter().next().unwrap().0);
        // eprintln!("bestst: {:?}", best_st);
