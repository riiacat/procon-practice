...
fn main() {
    ...
    let mut minus_table = vec![vec![0; q]; 10];

    for a in 0..10{
        let mut minus = a;
        for l in 0..q{
            minus_table[a][l] = minus;
            minus *= 10;
            minus %= MOD;
        }
    }
    ...
    for i in 0..q{
        ...
        match qid {
            ...
            2 => {
                let l = vq.len();
                let head = vq.pop_front().unwrap();

                let minus = minus_table[head][l-1];
                // res += (MOD-minus);
                // eprintln!("head, l, minus: {:?}, {:?}, {:?}", head, l, minus);
                res += (MOD-minus);
                // res -=minus;
                res %= MOD;
            }
