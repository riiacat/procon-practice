...
fn main() {
    ...
    let mut cnts: Vec<HashMap<usize, usize>> = vec![HashMap::default(); n+1];
    let mut box_st: Vec<HashSet<usize>> = vec![HashSet::default(); 200000+1];
    ...
    for i in 0..q{
        ...
        match qid {
            1 => {
                ...
                box_st[i].insert(j);
                cnts[j].entry(i).and_modify(|e| *e += 1).or_insert(1);
            }
            2 => {
                input![i: usize];
                let mut cnts = cnts[i].iter().collect_vec();
                cnts.sort_by_key(|(n, _)| *n);
            ...
            3 => {
                ...
                let mut boxes = box_st[i].iter().collect_vec();
                boxes.sort();
