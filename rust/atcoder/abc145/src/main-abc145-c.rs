...
fn main() {
    ...
    for v in vs{
        for i in 0..n-1{
            sum += (((xy[v[i]].0 - xy[v[i+1]].0).pow(2) + (xy[v[i]].1 - xy[v[i+1]].1).pow(2))as f64).sqrt();
        }
    }
...
fn dfs(current: usize, before: Option<usize>, n: usize) -> Vec<Vec<usize>>{
    ...
    //         let (x1, y1) = obs[before];
    //         let (x2, y2) = obs[i];
    //         let dist = (((x1-x2).pow(2) + (y1-y2).pow(2)) as f64).sqrt();

    //         eprintln!("{} -> {}, d={}", before, i, dist);
    //         let (dist2, count) = dfs(current | (1 << i), Some(i), obs);
    //         res.0 += dist + dist2;
    //         res.1 += count;
    //     }else{
    //         let (dist, count) = dfs(current | (1 << i), Some(i), obs);
    //         res.0 += dist;
    //         res.1 += count;
    //     }
    // }

    // if (0..obs.len()).all(|i| (current >> i) & 1 == 1){
    //     res.1 += 1;
    // }
    // res
    ...
    for i in 0..n{
        ...
        // eprintln!("{:?} -> {}", before, i);
        let mut new_res = dfs(current | (1 << i), Some(i), n);
        for v in new_res.iter_mut(){
            v.push(i);
        }
