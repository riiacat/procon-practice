...
// #[fastout]
fn main() {
    input![n: usize, t: usize, ab:[(isize, isize); n]];
    //new type

    let t = t-1;

    let ab_max = ab.iter().map(|tpl| tpl.1).max().unwrap();
    let extracted_ab = ab.iter().filter(|(a, b)| *b == ab_max)
        .max_by_key(|(a,_)| *a).unwrap();
    let (extracted_ab, (_, last_b))  = ab.iter().find_position(|tpl| *tpl == extracted_ab).unwrap();
    let mut new_ab  = vec![];
    for i in 0..n{
        if i != extracted_ab {
            new_ab.push(ab[i]);
    ...
    for i in 1..=n{
        for j in 0..t{
            let (a,b) = ab[i-1];
            dp[i][j] = max(dp[i-1][j], if j as isize -a  >= 0 {dp[i-1][j - a as usize] + b}else{0});
        }
    }
