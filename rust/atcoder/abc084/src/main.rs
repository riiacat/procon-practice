...
// #[fastout]
fn main() {
    input![n: usize, mut a:[usize; n],  mut b:[usize; n],  mut c:[usize; n]];
    //new type
    a.sort();b.sort();c.sort();

    let mut b_count = vec![];
    for i in 0..n{
        let idx = b.upper_bound(&a[i]);
        b_count.push(idx);
    }

    let mut c_count = vec![];
    for i in 0..n{
        let idx = c.upper_bound(&b[i]);
        c_count.push( c.len() - idx);
    }

    let mut c_count_cumsum = vec![0 as usize];
    c_count_cumsum.extend(c_count.iter().cumsum::<usize>());

    let mut ans = 0;

    for i in 0..n{
        ans += c_count_cumsum[n] - c_count_cumsum[b_count[i]];
    }
