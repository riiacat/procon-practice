...
fn main() {
    ...
    input![ xy: [(usize, usize); m]];

    let mut adjs = vec![vec![]; n];
    let mut adjs_rev = vec![vec![]; n];
    for (x, y) in xy{
        adjs[x-1].push(y-1);
        adjs_rev[y-1].push(x-1);
    }
    // eprintln!("adjs: {:?}", adjs);
    // eprintln!("adjs_rev: {:?}", adjs_rev);
    ...
    for i in 0..n{
        ...
        visit(i, &adjs, &mut visited, &mut sorted);
    }
    ...
    for i in 0..n-1{
        if adjs[sorted[i]].contains(&sorted[i+1]){
            continue;
        }else {
            println!("No");
            return;
    ...
    println!("Yes");
    let mut v = sorted.iter().copied().enumerate().sorted_by_key(|&(_, x)| x).map(|(i, _)| i+1).collect_vec();
    for v in v{
        print!("{} ", v);
    }
    println!();
...
fn visit(s: usize, adjs: &Vec<Vec<usize>>, visited: &mut Vec<bool>, sorted: &mut Vec<usize>){
    if visited[s]{return;}
    visited[s] = true;
    for adj in adjs[s].iter().copied(){
        if visited[adj]{continue;}
        visit(adj, adjs, visited, sorted);
    }
    sorted.push(s);
}
