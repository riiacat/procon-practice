...
fn main(){
    ...
    input![n: usize, c: usize, ta: [(usize, usize); n]];
    // com_init();

    // ある初期状態について考える
    let mut accums0 = vec![vec![]; 30];
    for b in 0..30{
        let mut v = 0;
        for (t, a) in ta.iter().copied(){
            match t {
                1 =>{v &= (a >> b) & 1},
                2 =>{v |= (a >> b) & 1},
                3 =>{v ^= (a >> b) & 1},
    ...
    let accums1 = accums1;
    eprintln!("accumslen: {}, {}", accums1.len(), accums1[0].len());

    let mut bits = vec![0_usize; 30];
    for i in 0..30{
        bits[i] = (c >> i) & 1;
    }

    let accums = [accums0, accums1];

    // eprintln!("bits: {:?}", bits);

    // eprintln!("accums0: {:?}", accums[0].iter().map(|v| v[0]).collect_vec());
    // eprintln!("accums1: {:?}", accums[1].iter().map(|v| v[0]).collect_vec());

    for i in 0..n{
        // eprintln!("bits: {:?}", bits);
        for b in 0..30{
            // eprintln!("bits: {}", bits[i]);
            bits[b] = accums[bits[b]][b][i];
        }
        let mut base = 1;
        let mut res = 0;
        for b in 0..30{
