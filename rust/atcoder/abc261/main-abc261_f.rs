...
fn main(){
    /*
    KEY 転倒数をいくつか考える。
異なるものについて組み合わせを考えると面倒なので、同じものについて考えて、母数から引く
undoするのに、SegTreeを毎回初期化するとTLEになるので、操作の系列が終わったら、逆回しで-1することで、操作列の数でundoできる
     */
    ...
    let mxc = c.iter().max().unwrap();
    let mxx = x.iter().max().unwrap();
    let mut c2x = vec![vec![]; mxc+1];
    // let mut x2c = vec![vec![]; mxc+1];
    for xc in x.iter().zip(c.iter()){
        // x2c[*xc.0].push(*xc.1);
        c2x[*xc.1].push(*xc.0);
    }


    #[cfg(debug_assertions)]{eprintln!("c2x", );}
    for r in c2x.iter().enumerate(){
        #[cfg(debug_assertions)]{eprintln!("{}, {:?}", r.0, r.1);}
    }

    // let mut sortedx = Vec::from_iter(x.iter().copied());
    // sortedx.sort();

    let mut st = SegTree::new(*mxx+1);
    for i in 0..mxx+1{
        st.update(i, 0, |a, b| Some(a.unwrap_or(0)+b.unwrap_or(0)));
    }

    let mut plus = 0;
    let select_fn = |a: &Option<usize>, b: &Option<usize>| Some(a.unwrap_or(0) + b.unwrap_or(0));
    for i in 0..n{
        plus+= st.query(&select_fn, x[i]+1, mxx+2).unwrap_or(0);
        st.update(x[i],st.query(&select_fn, x[i], x[i]+1).unwrap_or(0)+1 ,  |a, b| Some(a.unwrap_or(0)+b.unwrap_or(0)));
    }
    ...
    let mut minus = 0;
    for vs in c2x.iter(){
        if vs.len() <= 1{continue;}
        for i in 0..vs.len(){
            minus+= st.query(&select_fn, vs[i]+1, mxx+2).unwrap_or(0);
            st.update(vs[i],st.query(&select_fn, vs[i], vs[i]+1).unwrap_or(0)+1 ,  |a, b| Some(a.unwrap_or(0)+b.unwrap_or(0)));
        }
    ...
    #[cfg(debug_assertions)]{eprintln!("{:?}", (plus, minus));}
    println!("{}", plus-minus);
}
