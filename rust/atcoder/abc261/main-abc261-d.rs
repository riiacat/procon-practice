...
fn main(){
    ...
    input![n:usize, m:usize, x: [i64; n], cy:[(usize, i64); m]];

    let mut bs = vec![0_i64; n+1];
    for (c, y) in cy.iter().copied(){
        bs[c] = y;
    }

    let mut inf = -1_000_000_000_000_000_000_i64;
    ...
    for i in 1..=n{
        ...
        for j in 1..=n{
            chmax!(dp[i][j], dp[i-1][j-1] + x[i-1] + bs[j]);
            // chmax!(dp[i][j], dp[]);
