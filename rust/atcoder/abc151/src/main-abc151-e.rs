...
//     let v = vec![1, 2, 3];
//     let mut iter = next_permutation(v);
...
fn main() {
    ...
    for (idx, a) in a_s.iter().enumerate(){
        // if *a == old{continue;}
        // let idx = a_s.lower_bound(a);
        let idx2 = idx + 1;
        // eprintln!("1: {}, {}, {}", a, idx, idx2);
        let n_a = idx2 - idx;
        ...
        let a = (*a+MOD as i64) % MOD as i64;

        let p = ((a*n_a as i64) % MOD as i64)*com(n_lower, k as usize - 1) as i64 % MOD as i64;
        resp += p;
        resp %= MOD as i64;

        let m = ((a*n_a as i64) % MOD as i64)*com(n_upper, k as usize - 1) as i64 % MOD as i64;
        resm += m;
        resm %= MOD as i64;

        // eprintln!("2: {}, {}, {}, {}", n_lower, p, n_upper, m);

    }

    println!("{}", ((resp + MOD as i64)- resm) % MOD as i64);
}
