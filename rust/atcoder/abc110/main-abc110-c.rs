...
fn main(){
    ...
    input![s: AsciiString, t: AsciiString];

    let mut targets = vec![None; 26];
    let mut tgtst = HashSet::new();
    for i in 0..s.len(){
        let s = (s[i].as_byte()-97) as usize;
        let t = (t[i].as_byte()-97) as usize;

        if let Some(tgt) = targets[s]{
            if tgt != t{println!("No"); return;}
        }else{
            if tgtst.contains(&t){println!("No"); return;}
            targets[s] = Some(t);
            tgtst.insert(t);
        }
    }
    ...
    for (s, t) in targets.iter().copied().enumerate(){
