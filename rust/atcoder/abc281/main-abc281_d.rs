...
fn main(){
    ...
    let moda = a.iter().map(|&a| (a%d, a)).collect_vec();
    #[cfg(debug_assertions)]{eprintln!("moda: {:?}", moda);}
    let minf = -1_000_000_000_000;
    let mut dp = vec![vec![vec![minf; d as usize]; k+1]; n+1];
    ...
    for i in 0..n{
        ...
        #[cfg(debug_assertions)]{eprintln!("before: {i}", );}
        for r in dp[i].iter(){
            #[cfg(debug_assertions)]{eprintln!("{:?}", r);}
        }
        ...
        for j in 1..=k{
            ...
            for k in 0..d as usize{
                chmax!(dp[i+1][j][(k+moda as usize)%d as usize],dp[i][j-1][k]+a);
            }
        }
    }
