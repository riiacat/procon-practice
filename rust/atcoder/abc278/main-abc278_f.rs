...
fn main(){
    ...
    {
        let mut ddp = dp.lock().unwrap();
        *ddp = vec![vec![None ; n]; 1<<n];
    }

    let mut valid_st = vec![vec![]; n];

    for i in 0..n{
        for j in 0..n{
            if i == j {continue;}
            if *s[i].last().unwrap() == s[j][0]{
                valid_st[i].push(j);
            }
        }
    }

    let first_win = calc(0, 0, n, &s, &valid_st);
    println!("{}", if first_win {"First"}else{"Second"});
}

fn calc(state: usize, old: usize, n: usize, s: &Vec<Vec<char>>, valid_st: &Vec<Vec<usize>>) -> bool{
    {
        let ddp = dp.lock().unwrap();
        if let Some(a) = ddp[state][old]{return a;}
    }

    if state == (1<<n)-1{return false;}

    let tmp = (0..n).collect_vec();
    let valids = if state == 0{
        &tmp
    }else{&valid_st[old]};

    let mut any_false = false;
    for &new in valids.iter(){
        if state & (1<<new) != 0{continue;}
        any_false |= !calc(state|(1<<new), new, n, s, valid_st);
    }
