...
fn main(){
    ...
    for i in 0..q{
        ...
        match id{
            ...
            2 => {
                input![j: usize, p: usize];
                let j =j-1;
                // #[cfg(debug_assertions)]{eprintln!("{:?}", (j, p));}
                if accums[j].is_empty(){
                    // #[cfg(debug_assertions)]{eprintln!("1: {:?}", (j, i, p));}
                    accums[j].push((i, p));
                }else{
                    let old =accums[j].last().unwrap().1;
                    // #[cfg(debug_assertions)]{eprintln!("2: {:?}", (j, i, p));}
                    accums[j].push((i, p + old));
                }
            },
            3 => {
                input![i: usize];
                ...
                // #[cfg(debug_assertions)]{eprintln!("{:?}", (called1, last_called, last_v));}
                // #[cfg(debug_assertions)]{eprintln!("{:?}", accums[i]);}
                if !called1{
                    println!("{}", a[i]+accums[i].last().unwrap_or(&(0,0)).1);
                }else{
                    let pos = accums[i].upper_bound_by_key(&last_called, |v|{
                        v.0
                    });
                    if pos == 0{
                        println!("{}", last_v+accums[i].last().unwrap_or(&(0,0)).1);
                    }else{
                        let all = accums[i].last().unwrap().1;
                        let minus = accums[i][pos-1].1;
                        println!("{}", last_v+all-minus);
                    }
                }
            },
            _ => panic!()
