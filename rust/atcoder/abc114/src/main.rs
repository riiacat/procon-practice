...
fn main() {
    ...
    for n in 2..=n{
        let mut n = n as i64;
        while n > 1{
            let div = f_p[n as usize] as i64;
            n /= div;


            if let Some(v) = m.get_mut(&div){
                *v += 1;
            }else{
                m.insert(div, 1);
            }
    ...
    for (_,c) in m {
        n2 += if c >= 2 {1}else{0};
        n4 += if c >= 4 {1}else{0};
        n14 += if c >= 14 {1}else{0};
        n74 += if c >= 74 {1}else{0};
        n24 += if c >= 24 {1}else{0};
    }

    // eprintln!("n3: {}, n4: {}", n3, n4);
    println!("{}", n74 + n14 * max!(0, n4 - 1) + n24 * max!(0, n2-1) + n4 * (n4-1) *(n2- 2) / 2);
}
