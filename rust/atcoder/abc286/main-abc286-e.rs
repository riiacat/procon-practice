...
fn main(){
    ...
    input![n: usize, a: [usize; n], s: [Chars; n]];
    input![q:usize, st: [(usize, usize); q]];

    let mut adjs_mat = vec![vec![false;n]; n];
    let mut adjs = vec![vec![]; n];
    for i in 0..n{
        for j in 0..n{
            adjs_mat[i][j] = s[i][j] =='Y';
            if adjs_mat[i][j]{adjs[i].push(j)}
        }
    }
    ...
    for k in 0..n{
        for i in 0..n{
            for j in 0..n{
                ...
                chmin!(dist[i][j], (d0+d1, a0+a1));
            }
        }
    }

    for (s,t) in st.iter().copied(){
        let (d, v) =  dist[s-1][t-1];
        if d >= 500{println!("Impossible");}
        else{
            println!("{} {}", d, -v + a[s-1] as i64);
        }
    }
