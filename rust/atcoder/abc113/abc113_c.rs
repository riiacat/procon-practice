...
fn main(){
    ...
    // println!("Hello, world!");
    input![n: usize,m: usize, py: [(usize, usize); m]];

    let mut ken_shi = vec![vec![]; n];

    for (i, &(p, y)) in py.iter().enumerate(){
        ken_shi[p-1].push((y, i));
    }
    ...
    for (i, ken_shi) in ken_shi.iter().enumerate(){
        let pre = format!("{:06}", i+1);

        for (j ,&(_, shi_no)) in ken_shi.iter().enumerate(){
            let suf = format!("{:06}", j+1);
            res[shi_no] = format!("{}{}", pre, suf);
        }
    }
