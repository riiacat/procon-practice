...
fn main() {
    input![n:usize];
    let size =(10.pow(6 as u32)) as usize;
    ...
    for i in 1..size{
        ...
        for j in 2..size{
            let b = a * j;
            if b <= is_prime.len(){is_prime[b-1]=false;}
            else{break;}
        }
    }
    eprintln!("n_prime: {}", primes.len());

    let n = n as u128;
    let mut res = 0;
    for i in 0..primes.len(){
        if primes[i] > n as usize{break;}
        for j in i+1..primes.len(){
            let mut p = primes[i] as u128; let q = primes[j] as u128;
            let init_p = p;
            if p > n{break;}
            p *= q;
            if p > n{break;}
