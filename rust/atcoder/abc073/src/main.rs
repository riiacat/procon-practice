...
// #[fastout]
fn main() {
    input![n: usize, m: usize, r: usize, rs: [usize; r], abc: [(usize, usize, usize); m]];
    //new type

    let mut lens = vec![vec![std::usize::MAX / 100; n]; n];
    ...
    for k in 0..n {
        for i in 0..n {
            for j in 0..n {
                chmin!(lens[i][j], lens[i][k] + lens[j][k]);
            }
        }
    }
    ...
    let rs = rs.iter().map(|r| r-1).collect_vec();

    let mut vecs = Vec::new();
    permutations(&mut vecs, Vec::new(), 0, &rs);

    // eprintln!("{:?}", vecs);
    let mut ans = std::usize::MAX / 100;
    for vec in vecs{
        let mut mid = 0;

        for i in 0..rs.len()-1{
            mid += lens[vec[i]][vec[i+1]];
        }
...
fn permutations(vecs: &mut Vec<Vec<usize>>, cur_vec: Vec<usize>, current: usize, rs:& Vec<usize>){
    // eprintln!("{:b}", current);
    if current == 2usize.pow(rs.len() as u32) - 1 {
        vecs.push(cur_vec);
        return;
    }
    ...
    for i in 0..rs.len(){
        ...
        new_vec.push(rs[i]);
        permutations(vecs, new_vec, current | (1 << i), rs);
    }
}
