...
// #[fastout]
fn main(){
    /* KEY 累積的な状態を持つことから、尺取り的に解くことができる。また、適当にhashを累積で取ることで一致判定をO(1)ですることができる
（この場合は任意の区間の一致判定ができそうと思ったが、setの状態が個数に応じて変わるので、無理）
     */
    // let mut stdin = LineSource::new(BufReader::new(io::stdin()));
    // macro_rules! input(($($tt:tt)*) => (proconio::input!(from &mut stdin, $($tt)*)));

    // println!("Hello, world!");
    input![n: usize, a: [usize; n], b:[usize; n], q: usize, xy: [(usize, usize); q]];
    ...
    let mut st_unseen_a = HashSet::new();
    // let mut st_unseen_b = HashSet::new();
    ...
    for aidx in 0..n{
        ...
        st_a.insert(a);
        if !st_b.contains(&a){st_unseen_a.insert(a);}

        while bidx<n && st_a.contains(&b[bidx]){
            st_b.insert(b[bidx]);
            if st_unseen_a.contains(&b[bidx]) && st_unseen_a.len()==1{
                start = bidx;
            }
            st_unseen_a.remove(&b[bidx]);
            bidx+=1;
        }

        if st_unseen_a.len()!=0{
            start = bidx;
        }
        ranges.push((start, bidx));


        #[cfg(debug_assertions)]{eprintln!("{:?}", ranges.last().unwrap());}
        #[cfg(debug_assertions)]{eprintln!("st_a: {:?}", st_a)}
        #[cfg(debug_assertions)]{eprintln!("at_b: {:?}", st_b);}
        #[cfg(debug_assertions)]{eprintln!("st_a_unseen: {:?}", st_unseen_a);}
        // #[cfg(debug_assertions)]{eprintln!("at_b_unseen: {:?}", st_unseen_b);}
    }

    #[cfg(debug_assertions)]{eprintln!("{:?}", ranges);}

    for &(x, y) in xy.iter(){
        let x=x-1;let y=y-1;
        if ranges[x].0 <= y && y < ranges[x].1{
            println!("Yes");
        }else{
            println!("No");
        }
