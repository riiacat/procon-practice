...
fn dfs(cur: usize, prev: usize, adjs: &Vec<Vec<usize>>, res: &mut Vec<(usize, usize)>, id: &mut usize){
    // eprintln!("cur: {}", cur);
    let (mut l, mut r) = (2000000, 0);
    for &to in adjs[cur].iter(){
        if to==prev{continue;}
        dfs(to, cur, adjs, res, id);
        // eprintln!("{}->{}, lr: {:?}", cur, to, res[to]);
        chmin!(l, res[to].0);
        chmax!(r, res[to].1);
    }

    if cur != 0 && adjs[cur].len()==1{
        *id+=1;
        res[cur] = (*id, *id)
    }else{
        res[cur] =(l,r);
