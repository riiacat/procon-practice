...
fn main(){
    ...
    for i in 0..n{
        if i % 2==0{
            cnt1.entry(vs[i]).and_modify(|a| *a+=1).or_insert(1_usize);
        }else{
            cnt2.entry(vs[i]).and_modify(|a| *a+=1).or_insert(1_usize);
        }
    }

    let mut cnt1 = cnt1.iter().collect_vec();
    let mut cnt2 = cnt2.iter().collect_vec();

    cnt1.sort_by_key(|(v, cnt) |Reverse(**cnt));
    cnt2.sort_by_key(|(v, cnt) |Reverse(**cnt));

    if cnt1[0].0 != cnt2[0].0{
        println!("{}", n-cnt1[0].1-cnt2[0].1);
    }else{
        println!("{}", min!(n-cnt1[0].1-if cnt2.len()==1{0}else{*cnt2[1].1}, n-if cnt1.len() >1 {*cnt1[1].1}else{0}-cnt2[0].1));
    }
