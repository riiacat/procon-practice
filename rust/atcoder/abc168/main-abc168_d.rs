...
fn main(){
    ...
    while !q.is_empty(){
        let nid = q.pop_front().unwrap();

        for &to in ajds[nid].iter(){
    ...
    if prev.iter().any(|prev| *prev < 0){
        println!("No");
    }else{
        println!("Yes");
        for p in prev.iter().dropping(1).copied(){
            println!("{}", p+1);
        }
    }

    // println!("{}", dp.iter().filter(|&&a| a < 2*1000000).count());
