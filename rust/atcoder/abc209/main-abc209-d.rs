...
fn main(){
    /* KEY
        木の2部グラフ性だけが必要で、lcaを求める必要はない。。lcaで距離を求める場合、lcaの部分は２倍されてるので偶奇には関係ないよ。
     */
    ...
    input![n: usize, q: usize, ab: [(usize, usize); n-1], cd: [(usize, usize); q]];

    let mut pars = vec![0; n+1];
    ...
    while nvis < n{
        let (nid, d) = s.pop_back().unwrap();

        for to in adjs[nid-1].iter().copied(){
            if depth[to+1] != -1{continue;}
            depth[to+1]=d+1;
            nvis+=1;
            pars[to+1]=nid;
            s.push_back((to+1, d+1));
        }
    }
    ...
    for (mut c,mut d) in cd.iter().copied(){
        let oric=c;let orid=d;
        if depth[c] < depth[d]{swap(&mut c, &mut d)}
        let dd = depth[c] - depth[d];

        for i in 0..20{
            if dd & (1<<i) == 0{continue;}
            c = dpd[c][i];
        }
        ...
        let d = depth[oric]+depth[orid]-(2*depth[lcm]) as i64;
        // eprintln!("lcm, d: {}, {}", lcm, d);
        println!("{}", if d % 2 == 0{"Town"}else{"Road"});
    }
