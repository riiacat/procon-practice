...
fn main(){
    ...
    input![n: usize, mut k: usize, x: [i64; n]];
    ...
    for i in 0..n{
        if x[i] == 0{k-=1;}
        else if x[i] > 0 {xp.push(x[i] as usize)}
        else {xm.push((-x[i]) as usize)}
    }
    let k =k;
    xm.sort();
    ...
    let mut res = 1_000_000_000_000_000_000_usize;
    for l in 0..=k{
        let r = k-l;

        if xm.len() < l{continue;}
        if xp.len() < r{continue;}


        let a = if l > 0{xm[l-1]}else{0};
        let b = if r > 0{xp[r-1]}else{0};
