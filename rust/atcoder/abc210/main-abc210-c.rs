...
fn main(){
    ...
    for i in 0..k{
        let old = *cnts.get(&c[i]).unwrap_or(&0_usize);
        if old == 0{ks += 1;}
        cnts.insert(c[i], old+1);
    }
    let mut res = ks;
    eprintln!("{}", ks);
    ...
    for i in k..n{
        ...
        let old0 = *cnts.get(&c[i]).unwrap_or(&0_usize);
        let old1 = *cnts.get(&c[i-k]).unwrap();
        if old0 == 0{ks+=1;}
        if old1 == 1{ks-=1;}
        chmax!(res, ks);
        cnts.insert(c[i], old0+1);
        cnts.insert(c[i-k], old1-1);
    }
