...
// #[fastout]
fn main(){
    /* KEY 全体をソートするとかなり時間がかかるが、部分的にしか要らない場合は計算量が落とせる場合がある
     */
    // let mut stdin = LineSource::new(BufReader::new(io::stdin()));
    // macro_rules! input(($($tt:tt)*) => (proconio::input!(from &mut stdin, $($tt)*)));

    // println!("Hello, world!");
    input![n: usize, q: usize, x: [usize; n], ab:[(usize, usize); n-1], vk: [(usize,usize); q]];
    ...
    let mut resset = vec![vec![]; n];
    dfs(0, 0, &adjs, &x, &mut resset);

    // let mut resset

    for &(v, k) in vk.iter(){
        println!("{}", resset[v-1][k-1]);
    }
}

fn dfs(cur: usize, par: usize, adjs: &Vec<Vec<usize>>, x: &Vec<usize>, res: &mut Vec<Vec<usize>>){

    let mut newres = vec![x[cur]];
    for &to in adjs[cur].iter(){
        if par==to{continue;}
        dfs(to, cur, adjs, x, res);
        newres.extend(res[to].iter());
    }
    newres.sort_by_key(|a| Reverse(*a));
    newres = newres.iter().copied().take(20).collect_vec();
    res[cur] = newres;
}
