...
// #[fastout]
fn main(){
    /*KEY 2つの数字のxorは1になることを利用すれば、O(1)で解けるよ
     */
    ...
    for bt in 0..40{
        let interval = (1<<(bt+1));
        #[cfg(debug_assertions)]{eprintln!("interval: {:?}", (bt, interval));}
        let na = if a == 0{0}else{
            let n = a/interval;
            let r = a % interval;
            n*interval/2 + max!(0, r - interval/2)
        };
        let nb = {
            let n = (b+1)/interval;
            let r = (b+1) % interval;
        ...
        #[cfg(debug_assertions)]{eprintln!("{:?}", (bt, na, nb, n));}
        if (n&1)==1{
            res += (1<<bt);
        }
    }
