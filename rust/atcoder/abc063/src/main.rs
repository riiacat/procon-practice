...
// #[fastout]
fn main() {
    input![n: usize, t:usize, a: [i64; n]];

    let mut right_higher = (10000000, 0);
    let mut otoku = -1;
    let mut otokus = vec![];

    for (idx, v) in a.iter().rev().enumerate(){
        if right_higher.1 < *v {
            right_higher = (idx, *v);
        }

        let o2 = right_higher.1 - v;
        if o2 > otoku{
            otoku = o2;
            otokus = vec![(idx, right_higher.0)];
        }else if o2 == otoku {
            otokus.push((idx, right_higher.0));
        }
        // chmax!(right_higher, *a);
    }

    // eprintln!("{}, {:?}", otoku, otokus);
    println!("{}", otokus.len());
}
