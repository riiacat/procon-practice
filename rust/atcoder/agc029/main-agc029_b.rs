...
// #[fastout]
fn main(){
    /* KEY 最大から作るタイプの貪欲。作れるものが一意に定まる順番がポイント
     */
    ...
    for &a in a.iter(){
        let mut found =false;
        for &b in nibeki.iter(){
            if a >= b{continue;}
            let &c = cnts.get(&(b-a)).unwrap_or(&0);

            if c > 0{
                #[cfg(debug_assertions)]{eprintln!("{:?}", (a, b-a));}
                cnts.insert(b-a, c-1);
                res+=1;
                found=true;
                break;
        ...
        if !found{
            cnts.entry(a).and_modify(|c| *c+=1).or_insert(1);
        }
    }
