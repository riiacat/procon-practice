...
fn main() {
    ...
    input![t: usize, ss: [Chars; t]];

    // let mut targets = HashSet::from_iter(vec![]);
    let atc = "atcoder".to_string();
    let atc_chars = "atcoder".to_string().chars().collect_vec();
    for s in ss {
        let string_s = s.iter().collect::<String>();
        // eprintln!("{}", string_s);
        if string_s > atc{
            println!("0");
            continue;
