...
fn main(){
    ...
    let mut sosas = vec![];
    while l > 0{
        if sosas.len() > 2000{println!("No"); return ;}
        #[cfg(debug_assertions)]{eprintln!("cur: {:?}", cur);}

        let mut old = 0;
        let mut renzokus = vec![];

        let mut i = 0;
        let mut poss = vec![None; n+1];

        let mut ok = true;
        for i in 0..cur.len()-1{
            if cur[i] > cur[i+1]{ok=false; break;}
        }
        if ok{break;}

        while i < l{
            if i < l-1 && cur[i]+1==cur[i+1]{
                poss[cur[i]] = Some(i);
                poss[cur[i+1]] = Some(i+1);
                renzokus.push((cur[i], cur[i+1], i));
                i+=2;
            }else if cur[i] < old{
                println!("No");
                return ;
        ...
        if renzokus.is_empty(){break;}

        renzokus.sort_by_key(|v| Reverse(v.0));
        let (l1, l2, idx) = renzokus[0];
        #[cfg(debug_assertions)]{eprintln!("rensoku: {:?}", renzokus[0]);}

        let mut next = vec![];
        sosas.push((idx, poss[l1-1].map(|idx2| if poss[l1].unwrap() < idx2 {idx2-2}else{idx2} )));
        #[cfg(debug_assertions)]{eprintln!("new_sosa: {:?}", (idx, poss[l1-1]));}

        for i in 0..cur.len(){
            if cur[i] == l1 || cur[i] ==l2 || cur[i] > l2{continue;}
            next.push(cur[i]);
        }
        cur = next;
        l = cur.len();
    ...
    println!("Yes");
    println!("{}", sosas.len());
    for (i, j) in sosas.iter(){
        println!("{} {}", i+1, j.map(|j| j+1).unwrap_or(0));
    }
