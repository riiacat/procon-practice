...
// #[fastout]
fn main(){
    /*KEY 逆から考える。また、にぶたんで頭から追っていくのでもよかった。
最小と最大においては、 res(x) >= 2をOKとした時に最小のokがもとまる。
逆にres(x) <= 2をokとした時に最大のokがもとまる。
愚直にシミュレーションすればok。
最大の答えは 2+kx max(a)で十分。（毎回の脱落者の最大値を取る）
     */
    ...
    let mut lr = (2, 3);
    for &a in a.iter().rev().dropping(1){

        // if a % lr.0 || lr.1 < a{
        //     println!("-1"); return;
        // }
        let mn = ((lr.0/a) + if lr.0 % a == 0{0}else{1})*a;
        let mx = ((lr.1/a)) * a;
        #[cfg(debug_assertions)]{eprintln!("mnmx: {:?}", (mn, mx));}
        if mn > lr.1 || mx < lr.0{
            println!("-1"); return;
        }

        lr = (mn, mx+(a-1));
        #[cfg(debug_assertions)]{eprintln!("a: {}, lr: {:?}", a, lr);}
    }
