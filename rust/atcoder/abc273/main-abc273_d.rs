...
// #[fastout]
fn main(){
    /* KEY 行列で個別に座標圧縮したものを持つ。今の位置に一番近いものがわかればいいので、ただのにぶたんでもよかった
     */
    // let mut stdin = LineSource::new(BufReader::new(io::stdin()));
    // macro_rules! input(($($tt:tt)*) => (proconio::input!(from &mut stdin, $($tt)*)));

    // println!("Hello, world!");
    input![h: i64, w: i64, rs: i64, cs:i64, n:usize, mut rc: [(i64, i64); n], q:usize, dl: [(char, i64); q]];
    rc.sort();

    let mut rwalls = HashMap::new();
    let mut old = -1;

    #[cfg(debug_assertions)]{eprintln!("rc1: {:?}", rc);}
    for &(r, c) in rc.iter(){
        if r as i64 != old{
            rwalls.insert(r, BTreeSet::new());
        }
        rwalls.entry(r).and_modify(|m| {m.insert(c);} );
        old = r as i64;
    }

    let mut cwalls = HashMap::new();
    rc.sort_by_key(|&(r,c)| (c, r));

    #[cfg(debug_assertions)]{eprintln!("rc2: {:?}", rc);}
    for &(r, c) in rc.iter(){
        if c as i64 != old{
            cwalls.insert(c, BTreeSet::new());
        }
        cwalls.entry(c).and_modify(|m| {m.insert(r);} );
        old = c as i64;
    }


    #[cfg(debug_assertions)]{eprintln!("rwalls: {:?}", rwalls);}
    #[cfg(debug_assertions)]{eprintln!("cwalls: {:?}", cwalls);}

    let mut cur =(rs, cs);

    for &(dir, l) in dl.iter(){
        let (r, c) = cur;
        match dir {
            'L' =>{
                if let Some(bt) = rwalls.get(&r){
                    if let Some(thresh) = bt.range(c-l..c).next_back(){
                        cur = (r, thresh+1);
                    }else{
                        cur = (r, max!(c-l, 1));
                    }
            ...
            'R' =>{
                if let Some(bt) = rwalls.get(&r){
                    if let Some(thresh) = bt.range(c+1..c+l+1).next(){
                        cur = (r, thresh-1);
                    }else{
                        cur = (r, min!(c+l, w));
                    }
            ...
            'U' =>{
                if let Some(bt) = cwalls.get(&c){
                    if let Some(thresh) = bt.range(r-l..r).next_back(){
                        cur = (thresh+1, c);
                    }else{
                        cur = (max!(r-l, 1), c);
                    }
            ...
            'D' =>{
                if let Some(bt) = cwalls.get(&c){
                    if let Some(thresh) = bt.range(r+1..r+l+1).next(){
                        cur = (thresh-1, c);
                    }else{
                        cur = (min!(r+l, h), c);
                    }
