...
fn main(){
    ...
    input![n:usize, m:usize, mut k:i64, a: [usize; m],  uv: [(usize, usize); n-1]];
    // chmax!(k, -k);
    // let k = k as usize;

    let mut weights = vec![0_i64; n-1];
    let mut adjs = vec![vec![]; n];
    for (eid, (u,v)) in uv.iter().copied().enumerate(){
        adjs[u-1].push((eid, v-1));adjs[v-1].push((eid, u-1));
    }

    for i in 0..m-1{
        let s = a[i]; let t=a[i+1];
        let s=s-1;let t=t-1;

        let mut st = VecDeque::new();
        let mut vsted = vec![false; n];
        vsted[s] = true;
        st.push_back((s, vec![]));
        let mut ret_path = vec![];
        loop{
            let (nid, mut path) = st.pop_back().unwrap();
            if nid == t{ret_path = path; break;}

            for (eid, to) in adjs[nid].iter().copied(){
                if vsted[to]{continue;}
                vsted[to] = true;
                let mut newp = path.clone();
                newp.push((eid, to));
                // weights[eid] += 1;
                st.push_back((to, newp));
            }
        }
        // eprintln!("path: m={}, {:?}", i, ret_path);
        for (eid, nid) in ret_path.iter().copied(){
            weights[eid]+=1;
        }
    }
    ...
    let s = weights.iter().sum::<i64>();
    let target = k;

    let n = n-1;

    let mut dp = vec![vec![0_usize;3*(100000+1)]; 2];
    let base = 150000_i64;
    ...
    for i in 0..2{dp[i][base as usize] = 1;}

    let md = 998244353;
    for i in 0..n{
        let mut cur = i%2;
        let mut nxt = 1-cur;
        for k in 1..=2*(100000+1){
            let k = k as i64 - 100000;
            dp[nxt][(k+base) as usize] = dp[cur][(k+base-weights[i]) as usize] + {dp[cur][(k+weights[i]+base) as usize]};
            dp[nxt][(k+base)as usize] %= md;
        }
        // eprintln!("i={}", i);
        // for row in dp.iter(){
        //     eprintln!("{:?}", row);
    ...
    println!("{}", dp[n%2][(target +base as i64) as usize]);
