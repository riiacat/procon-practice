...
lazy_static! {
    ...
    static ref dp: Mutex<Vec<Vec<Option<bool>>>> = Mutex::default();
}
...
fn main(){
    ...
    let w_cumsum = {
        let mut c = vec![0];
        c.extend(w.iter().copied().cumsum::<usize>());
        c
    };

    let mut ends = vec![0_usize; n];
    let mut n_load = vec![0_usize; n];

    for i in 0..n{
        // eprintln!("searching: {}", i);
        let w_until_end = w_cumsum[n] - w_cumsum[i];
        if w_until_end >= x{
            //TODO 二部たんで探す
            let mut ng = i;
            let mut ok = n;
            while ok-ng > 1{
                let mid = (ok+ng)/2;
                if w_cumsum[mid] - w_cumsum[i] >= x{ok=mid}else{ng=mid}
            }
            ends[i] = ok-1;
            n_load[i] = ok-i;
        }else{
            let x = x-w_until_end;
            let n_loop = x / w_cumsum.last().unwrap();
            let rest = x % w_cumsum.last().unwrap();

            if rest == 0{
                ends[i] = n-1;
                n_load[i] = (n-i)+n_loop*n;
            }else{
                let end = w_cumsum.lower_bound(&rest);
                ends[i] = end-1;
                n_load[i] = (n-i) + n_loop*n+ end;
            }
        }
    }

    eprintln!("ends: {:?}", ends);
    eprintln!("n_load: {:?}", n_load);

    let mut prev_i = vec![-1; n];
    ...
    eprintln!("prev_i: {:?}", prev_i);
    eprintln!("cur: {:?}", cur);

    let n_until_loop = prev_i[cur] as usize;
    let loop_l = starts.len()-n_until_loop;

    eprintln!("n_until: {}, loopl: {}", n_until_loop, loop_l);

    for &k in k.iter(){
        // eprintln!("k: {}", k);
        if k <= starts.len() {
            println!("{}", n_load[starts[k-1]]);
            continue;
        }

        let k = k-n_until_loop;
        let n_loop = k / loop_l;
        let rest = k % loop_l;

        // eprintln!("k: {}, n_loop: {}, rest: {}", k, n_loop, rest);

        println!("{}", n_load[starts[(n_until_loop+starts.len()+rest-1)%starts.len()]]);
    }


    // eprintln!("acnt: {}, bcnt: {}", acnt, bcnt);
