...
// #[fastout]
fn main(){
    /* KEY 全体から部分をひく。普通に mod_comを使うには nが大きいが、
    a,bが小さいことからオーソドックスな組み合わせの公式を使うことができる
     */
    ...
    for i in 0..a{
        coma *= n-i;
        coma %=MOD;
        coma *= modinv((a-i) as i64, MOD as i64) as usize;
        coma %=MOD;
    }
    ...
    for i in 0..b{
        comb *= n-i;
        comb%=MOD;
        comb *= modinv((b-i) as i64, MOD as i64) as usize;
        comb %=MOD;
    }
