...
fn main() {
    // println!("Hello, world!");
    input![n: usize, d: [usize; n], m: usize, t: [usize; m]];

    let mut cnts1 = HashMap::new();
    let mut cnts2 = HashMap::new();

    for d in d.iter().copied(){
        cnts1.entry(d).and_modify(|a| *a+=1).or_insert(1_usize);
    }
    for d in t.iter().copied(){
        cnts2.entry(d).and_modify(|a| *a+=1).or_insert(1_usize);
    }

    for (k,v) in cnts2.iter(){
        if cnts1.get(k).unwrap_or(&0) >= v{
