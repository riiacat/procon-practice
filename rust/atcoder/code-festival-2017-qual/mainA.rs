...
fn main() {
    ...
    for a in a.iter(){
        for a in a.iter(){
            let  e = chars.entry(*a).or_insert(0 as isize);
            *e += 1;
        }
    }
    ...
    let is_h_odd = h % 2 == 1;
    let is_w_odd = w % 2 == 1;
    let is_exist_center = is_h_odd && is_w_odd;
    let mut minus_count = 0 as isize;

    for i in 0..h/2 * (w /2){
        match chars.iter_mut().find(|(k, v)| **v >= 4) {
            Some((k, v)) => {
                *v -= 4;
                minus_count += 4;
            }
    ...
    if is_h_odd{
        ...
        for i in 0..leng{
            match chars.iter_mut().find(|(k, v)| **v >= 2) {
                Some((k, v)) => {
                    *v -= 2;
                    minus_count += 2;
                }
    ...
    if is_exist_center{
        match chars.iter_mut().find(|(k, v)| **v >= 1) {
            Some((k, v)) => {
                *v -= 1;
                minus_count += 1;
            }
    ...
    // eprintln!("h*w: {}, minus: {}", h *w, minus_count);
    println!("Yes");
