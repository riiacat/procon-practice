...
fn main(){
    ...
    // eprintln!("start_loop, vs={:?}, {:?}", start_loop, vs);
    ...
    let l = vs.len() - start_loop;
    let (n_loop, remained) = div_mod_floor(n-start_loop, l);

    // eprintln!("l: {}, n_loop: {}, remained: {}", l, n_loop, remained);

    let mut res = 0;
    for i in 0..(start_loop+remained){
        res+=vs[i];//res%=m;
    }

    let mut loop_sum = 0;
    for i in start_loop..vs.len(){
        // eprintln!("i: {}, vs[i]: {}", i, vs[i]);
        loop_sum+=vs[i]; //loop_sum%=m;
    }
