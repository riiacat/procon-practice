...
fn main(){
    ...
    input![t1: i64, t2: i64, mut a1: i64, mut a2: i64, mut b1: i64, mut b2: i64];

    if a1 > b1{swap(&mut a1,&mut b1); swap(&mut a2,&mut b2); }
    ...
    eprintln!("d1: {}, d2{}, s: {}, d1/s: {}, mod: {}", d1, d2, s, d1/(-s), d1%(-s));

    if d1 / (-s) == 0{
        println!("1");
    }else{
        println!("{}", (max!(0, d1/(-s) - 1))*2+ if (d1%(-s)) != 0{2}else{1} + 1);
    }
