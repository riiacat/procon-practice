...
fn main(){
    ...
    search(&(0..n).collect_vec(), n, &s, &mut res, 0);
...
fn search(targets: &Vec<usize>, n: usize, s: &Vec<Vec<char>>, res: &mut Vec<usize>, k: usize){
    // if k == n{
    ...
    // for i in 0..26{
    //     let c = AsciiChar::from_ascii(97_u8+i).unwrap().as_char();
    // }

    for &t in targets.iter(){
        if s[t].len() == k{res[t]=k; continue;}
        let idx = (AsciiChar::new(s[t][k]).as_byte() - 97) as usize;
        groups[idx].push(t);
    }
