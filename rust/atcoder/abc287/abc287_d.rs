...
fn main(){
    ...
    let mut ismatch0 = vec![true];
    let mut ismatch1 = vec![true];
    for i in 0..t.len(){
        ismatch0.push(*ismatch0.last().unwrap() && (s[i]==t[i] || s[i]=='?' || t[i]=='?'));
        ismatch1.push(*ismatch1.last().unwrap() && (s[sl-1-i] == t[tl-1-i] || s[sl-1-i]=='?' || t[tl-1-i]=='?'));
    }

    #[cfg(debug_assertions)]{eprintln!("match0: {:?}", ismatch0);}
    #[cfg(debug_assertions)]{eprintln!("match1: {:?}", ismatch1);}

    for i in 0..=t.len(){
        if ismatch0[i] && ismatch1[ismatch1.len()-1-i]{
            println!("Yes");
        }else{
            println!("No");
        }
