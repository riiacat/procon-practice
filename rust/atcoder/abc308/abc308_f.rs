...
fn main(){
    ...
    input![n: usize, m: usize, mut p: [usize;n], l: [usize; m], d:[i64; m]];
    p.sort();

    let mut ld = l.iter().copied().zip(d.iter().copied()).collect::<Vec<_>>();
    ld.sort_by_key(|(l,d)| -d);

    #[cfg(debug_assertions)]{eprintln!("{:?}", ld);}
    ...
    let mut bt = BTreeSet::from_iter(p.iter().copied().enumerate().map(|(i, v)| (v, i)));

    for &(l, d) in ld.iter(){
        // search p >= l from bt
        let mut it = bt.range((l, 0)..).next().copied();
