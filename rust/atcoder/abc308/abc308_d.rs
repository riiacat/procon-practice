...
fn main(){
    ...
    let snuke = vec!['s', 'n', 'u', 'k', 'e', 's'];

    let dhdw = [(0, -1), (1, 0), (-1,0), (0,1)];

    while !q.is_empty(){
        let (hh, ww) = q.pop_back().unwrap();
        #[cfg(debug_assertions)]{eprintln!("{:?}", (hh, ww));}
        if hh==h-1 && ww==w-1{
            println!("Yes"); return ;
        }

        for &(dh, dw) in dhdw.iter(){
            let new_h = hh as i64 + dh;
            let new_w = ww as i64 + dw;
            if new_h <0 || new_h >= h  as i64|| new_w <0 || new_w  >= w as i64{continue;}
            let new_h = new_h as usize;
            let new_w = new_w as usize;
            // vis[new_h as usize][new_w as usize] = true;
            if vis[new_h][new_w]{continue;}

            let next= snuke[snuke.iter().find_position(|v| **v == s[hh][ww]).unwrap().0+1];
            #[cfg(debug_assertions)]{eprintln!("{:?}", (new_h, new_w, next));}
            if s[new_h][new_w] != next{continue;}

            vis[new_h][new_w] = true;
            q.push_back((new_h, new_w))
        }
    }
