...
// #[fastout]
fn main(){
    /* KEY カウントしたものから組み合わせを列挙するよりも、ありうる値を全探索した方が早い場合もあるよ
     */
    ...
    for &a in a.iter(){
        cnts.entry(a).and_modify(|a| *a+=1).or_insert(1_usize);
    }
    ...
    for ak in 1..=amax{
        for bj in 1..=amax{
            if ak * bj > amax{break;}
            let ai = ak*bj;
            let delta = cnts.get(&ai).unwrap_or(&0)*cnts.get(&bj).unwrap_or(&0)*cnts.get(&ak).unwrap_or(&0);
            #[cfg(debug_assertions)]{eprintln!("{:?}", (ai, ak, bj, delta));}
            res += delta;
        }
    }
