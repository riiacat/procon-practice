...
fn main() {
    ...
    //
    // let mut ans = 0;
    // for a in &a.iter().as_slice()[..mid]{
    //     ans += a;
    // }
    //
    // if a.len() % 2 == 1 {
    //     ans += abs(a[mid]);
    //     for a in &a.iter().as_slice()[mid+1..]{
    //         ans -= a;
    //     }
    // }else{
    //     for a in &a.iter().as_slice()[mid..]{
    ...
    let mut dpmax = vec![-10000000000 as isize; n + 1];
    let mut dpmin = vec![10000000000 as isize; n + 1];

    dpmax[0] = 0;
    dpmin[0] = 0;

    for i in 1..=n{
        // eprintln!("{}",a[i-1] - dpmin[i-1]);
        // eprintln!("{}",dpmax[i-1] - a[i-1]);

        chmax!(dpmax[i], a[i-1] - dpmax[i-1]);
        chmax!(dpmax[i], dpmax[i-1] - a[i-1]);

        // chmin!(dpmin[i], a[i-1] - dpmin[i-1]);
        // chmin!(dpmin[i], dpmin[i-1] - a[i-1]);
        eprintln!("max-> {:?}", dpmax);
        eprintln!("min-> {:?}", dpmin);
    }
