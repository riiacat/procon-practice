...
fn main(){
    ...
    let mut zat = HashMap::new();

    for (a, b) in ab.iter().copied(){
        zat.entry(a).and_modify(|a| *a+=1).or_insert(1_i64);
        zat.entry(a+b).and_modify(|a| *a-=1).or_insert(-1_i64);
    }

    let mut zat = zat.iter().collect_vec();
    zat.sort();
    // eprintln!("{:?}", zat);
    ...
    let mut np = 0;
    for i in 0..zat.len()-1{
        let (d1, cnt) = zat[i];
        np += cnt;
        let (d2, _) = zat[i+1];

        // eprintln!("i, d1, d2, cnt, np: {}, {}, {}, {}, {}", i, d1, d2, cnt, np);
        cnts[np as usize] += d2-d1;
    }
