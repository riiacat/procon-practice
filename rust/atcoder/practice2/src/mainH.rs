...
pub mod internal_queue {
    ...
    #[cfg(test)]
    mod test {
        use super::super::internal_queue::SimpleQueue;
...
pub mod modint {
    ...
    #[cfg(test)]
    mod tests {
        use super::super::modint::ModInt1000000007;
...
// #[fastout]
fn main() {
    input![n: usize, d: isize, xy: [(isize, isize); n]];
    //new type
    ...
    let s = ts.satisfiable();
    println!("{}", if s {"Yes"}else{"No"});

    if !s {
        return;
    }
    for (b, (x,y)) in ts.answer().iter().zip(xy.iter()){
        let a = if *b {
            x
        }else{
            y
