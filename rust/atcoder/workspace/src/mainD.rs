...
// #[fastout]
fn main() {
    input![n: usize, w:usize, stp:[(usize, usize, usize); n]];

    let mut v = vec![0 as isize; 2_000_02];
    // let mut v = vec![0 as isize; 11];
    for i in 0..n{
        let (s, t, p) = stp[i];
    ...
    let mut com: Vec<isize> = v.iter().cumsum().collect_vec();

    if com.iter().all(|a| *a <= w as isize){
        println!("Yes")
    }else{
        println!("No")
    }
