...
fn main() {
    ...
    let mut ans_tate= vec![vec![0; w]; h];
    let mut ans_yoko = vec![vec![0; w]; h];
    let mut ans_naname = vec![vec![0; w]; h];
    let mut ans_tate2= vec![vec![0; w]; h];
    let mut ans_yoko2 = vec![vec![0; w]; h];
    let mut ans_naname2 = vec![vec![0; w]; h];
    ans[0][0] = 1;

    // let mut cum_tate = vec![0; w];
    // let mut cum_yoko = vec![0; h];
    // let mut cum_
    ...
    for hh in 0..h{
        for ww in 0..w{
            ...
            // ans[hh][ww] = ans_yoko2[hh][ww] + ans_tate2[hh][ww] + ans_naname2[hh][ww];
            if !(hh==0 && ww==0){
                ans[hh][ww] = ans_yoko[hh][ww] + ans_tate[hh][ww] + ans_naname[hh][ww];
                ans[hh][ww] %= MOD;
            }
            ...
            if ww + d == w || s[hh][ww + d] == '#'{
                ...
                ans_yoko[hh][ww+1] += ans_yoko[hh][ww] + ans[hh][ww];
                ans_yoko[hh][ww+1] %= MOD;
                // ans_yoko[hh][ww+1] += ans_yoko[hh][ww] + 1;
                // ans_yoko2[hh][ww+1] += ans_yoko[hh][ww + 1] + ans_yoko2[hh][ww];
                // ans_yoko2[hh][ww+1] += ans_yoko[hh][ww + 1] + ans[hh][ww];
            }

            if hh + d == h || s[hh + d][ww] == '#'{
                // break;
            }else{
                ans_tate[hh+1][ww] += ans_tate[hh][ww] + ans[hh][ww];
                ans_tate[hh+1][ww] %= MOD;
                // ans_tate[hh+1][ww] += ans_tate[hh][ww] + 1;
                // ans_tate2[hh+1][ww] += ans_tate[hh+ 1][ww] + ans[hh][ww];
            }

            if hh + d == h || ww + d == w || s[hh+d][ww+d] == '#'{

            }else{
                ans_naname[hh + 1][ww + 1] += ans_naname[hh][ww] + ans[hh][ww];
                ans_naname[hh + 1][ww + 1] %= MOD;
                // ans_naname[hh + 1][ww + 1] += ans_naname[hh][ww] + 1;
                // ans_naname2[hh + 1][ww + 1] += ans_naname[hh+1][ww+1] + ans[hh][ww];
            }
