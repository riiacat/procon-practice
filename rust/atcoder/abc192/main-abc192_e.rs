...
fn main(){
    ...
    // println!("Hello, world!");
    input![n: usize, m: usize, x: usize, y:usize, abtk: [(usize, usize, usize, usize); m]];

    let mut adjs = vec![vec![]; n];
    for &(a,b,t,k) in abtk.iter(){
        adjs[a-1].push((b-1, t, k));
        adjs[b-1].push((a-1, t, k));
    }
    #[cfg(debug_assertions)]{eprintln!("{:?}", "adjss");}
    for adj in adjs.iter(){
        #[cfg(debug_assertions)]{eprintln!("{:?}", adj);}
    }
    ...
    let mut cnt =0;
    while !bh.is_empty() && cnt<n{
        let (Reverse(d), nid) = bh.pop().unwrap();
        if mins[nid] != inf{continue;}
        mins[nid] = d;
        cnt+=1;
        #[cfg(debug_assertions)]{eprintln!("new: {:?}", (nid+1, d));}
        #[cfg(debug_assertions)]{eprintln!("{:?}", mins);}

        for &(to, t, k) in adjs[nid].iter(){
            if mins[to] != inf{continue;}
            let next = ((d/k)+if d%k==0{0}else{1})*k;
            bh.push((Reverse(next+t),to));
        }
    }
    #[cfg(debug_assertions)]{eprintln!("{:?}", mins);}
