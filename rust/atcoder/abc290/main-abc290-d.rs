...
fn main(){
    ...
    input![t: usize, ndk: [(usize,usize,usize,); t]];

    for (n, d, k) in ndk.iter().copied(){
        let k = k-1;
        let (gc, lc) = gcd_lcm(n, d);
        let n_per_rot = lc / d;
        // let delta = d % n;
        let nrot = k / n_per_rot;
        // let r = k % n_per_rot;
        // // eprintln!("n_per_rot: {}, delta: {}, nrot: {}, r: {}", n_per_rot, delta, nrot, r);
        // let res = if n >= d{
        //     // eprintln!("here0");
        //     // eprintln!("n-r*delta+nrot: {}-{}*{}+{}={}", n, r, delta, nrot, (n-r*delta)+nrot);
        //     ((n-r*delta)+nrot) % n
        // }else{
        //     // eprintln!("here1");
        //     ((0+r*delta)+nrot) % n
