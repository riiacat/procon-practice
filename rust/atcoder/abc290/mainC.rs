...
fn main() {
    ...
    let mut c_arr = HashMap::new();

    for a in a.iter() {
        let mut e = c_arr.entry(*a).or_insert(0);
        *e += 1;
        // c_arr.entry(*a).and_modify(|e| *e += 1);
    }

    let mut cnt = 0;
    let mut cumsum_arr = vec![];
    let mut res = 0;
    for i in 0..k {
        if let Some(n) = c_arr.get(&i) {
            cnt += n;
            cumsum_arr.push(cnt);
        } else {
            res = i - 1;
    ...
    // if cumsum_arr.is_empty() {
    //     println!("{:?}", 0);
    //     return;
    // }

    // for (i, cumsum) in cumsum_arr.iter().enumerate() {
    //     if *cumsum >= k {
    //         res = i;
    //     }
    // }
