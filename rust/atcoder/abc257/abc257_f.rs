...
// #[fastout]
fn main(){
    /*　KEY　経由地が足されるときは、経由してそのままゴールするパターンと
経由地から経由地に行ってゴールするパターンがあるぞ
     */
    ...
    let nodes = (0..n).map(|_| g.add_node(1)).collect_vec();

    let (adjs, have_mitei) = {
        let mut adjs = vec![vec![]; n];
        let mut have_mitei = vec![false; n];
        ...
        for &(u, v) in uv.iter(){
            if u==0{
                ...
                g.add_edge(nodes[v-1], nodes[u-1], ());
            }
        }
        (adjs, have_mitei)
    };

    let res1 = dijkstra(&g, nodes[0], None, |_| 1_usize);
    let res2 = dijkstra(&g, nodes[n-1], None, |_| 1_usize);

    #[cfg(debug_assertions)]{eprintln!("my node: {:?}", res1.get(&nodes[0]).unwrap());}

    let inf = 1_000_000_000_000_000_usize;
    let ds = res1.iter().map(|(nodeidx, d)| (if have_mitei[nodeidx.index()]{*d}else{inf}, nodeidx.index())).collect_vec();
    let mind = ds.iter().min().unwrap().0;
    let mind_sets: HashSet<_> = ds.iter().filter(|(d, nid)| *d==mind).map(|a| a.1).collect();
    let ds = res2.iter().map(|(nodeidx, d)| (if have_mitei[nodeidx.index()]{*d}else{inf}, nodeidx.index())).collect_vec();
    let mind2 = ds.iter().min().unwrap().0;
    let mind_sets2: HashSet<_> = ds.iter().filter(|(d, nid)| *d==mind).map(|a| a.1).collect();


    #[cfg(debug_assertions)]{eprintln!("{:?}", res1);}

    let init_d = *res1.get(&nodes[n-1]).unwrap_or(&inf);

    for i in 0..n{
        #[cfg(debug_assertions)]{eprintln!("{:?}",
        (
            init_d,
            min!(*res1.get(&nodes[i]).unwrap_or(&inf), mind + if mind_sets.contains(&i){0}else{1}) + res2.get(&nodes[i]).unwrap_or(&inf),
            min!(*res2.get(&nodes[i]).unwrap_or(&inf), mind2 + if mind_sets2.contains(&i){0}else{1}) + res1.get(&nodes[i]).unwrap_or(&inf),
        )
    );}
        let res = min!(init_d,
            min!(*res1.get(&nodes[i]).unwrap_or(&inf), mind + if mind_sets.contains(&i){0}else{1}) + res2.get(&nodes[i]).unwrap_or(&inf),
