...
fn main(){
    ...
    // println!("Hello, world!");
    input![w: usize, h: usize, n:usize, pq: [(i64, i64); n],
    A: usize, a: [i64; A], B: usize, b: [i64; B],
    ];
    ...
    for (p, q) in pq.iter(){
        ...
        cnts.entry((x, y)).and_modify(|c| *c+=1).or_insert(1_usize);
    }

    let n_area = (A+1)*(B+1);
    let mn = if cnts.len() == n_area{cnts.values().min().unwrap()}else{&0};
    let mx = cnts.values().max().unwrap();
