...
fn main(){
    ...
    // println!("Hello, world!");
    input![n: usize, m:usize, st: [(usize, usize); m], lr: [(usize, usize); n]];

    let mut adjs = vec![vec![]; n];
    let mut toe = vec![0_usize; n];
    for &(s, t) in st.iter(){
        adjs[s-1].push(t-1);
        toe[t-1]+=1;
    }
    ...
    for (i, &v) in toe.iter().enumerate(){
        if v==0{
            q.push((Reverse(lr[i].0), Reverse(lr[i].1), i));
        }
    }

    let mut res = vec![-1 as i64; n];
    let mut id = 1;
    while !q.is_empty(){
        let (Reverse(l), Reverse(r), nid) = q.pop().unwrap();
        if id < l || r < id{
            println!("No");
            return;
        }
        ...
        for &to in adjs[nid].iter(){
            toe[to]-=1;
            if toe[to] == 0{
                q.push((Reverse(lr[to].0), Reverse(lr[to].1), to));
            }
        }
    }

    if res.iter().any(|v| *v==-1){
        println!("No");
        return;
    }
