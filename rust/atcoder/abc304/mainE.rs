...
fn main(){
    ...
    k: usize, xy:[(usize, usize); k],
    q: usize, pq:[(usize, usize); q]
    ];
    ...
    let mut g: UnGraph<i32, ()> = petgraph::Graph::new_undirected();
    ...
    for (x,y) in uv.iter(){
        ...
        g.add_edge(nodes[y], nodes[x], ());
    }

    let connecteds = scc(&g);

    let sts = connecteds.iter().map(
        |nodes| nodes.iter().map(|node| node.index()
    ).collect::<HashSet<_>>()).collect_vec();

    let mut node2g = vec![0; n];
    ...
    let mut allno = false;
    let mut nggroups = HashSet::new();
    for &(x, y) in xy.iter(){
        let x=x-1; let y=y-1;
        if node2g[x] == node2g[y]{
            allno = true;
        }

        nggroups.insert((node2g[x], node2g[y]));
        nggroups.insert((node2g[y], node2g[x]));
    }
    ...
    for &(p, q) in pq.iter(){
        ...
        if nggroups.contains(&(node2g[p], node2g[q])){
            println!("No");
        }else{
            println!("Yes");
        }
