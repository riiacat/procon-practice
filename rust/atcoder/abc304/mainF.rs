...
fn main(){
    ...
    // let yakusus_set = HashSet::from_iter(yakusus.iter().cloned());

    #[cfg(debug_assertions)]{eprintln!("{:?}", yakusus);}

    for &yaku in yakusus.iter(){
        let mut res_this = 1_usize;
        for k in 0..yaku{
            let mut idx = k;
            let mut bai = true;
            while idx < n{
        ...
        for i in 1..yaku{
            ...
            if yaku % i == 0{
                yakusus2.push(i);
                if i*i != yaku{
                    yakusus2.push(yaku/i);
                }
            }
        }

        for &yaku in yakusus2.iter(){
            res_this+= MOD - dp[yaku];
            res_this%=MOD;
        }

        dp[yaku] = res_this;
    }
    #[cfg(debug_assertions)]{eprintln!("dp: {:?}", dp);}
