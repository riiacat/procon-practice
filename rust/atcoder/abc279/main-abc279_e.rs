...
fn main(){
    /* KEY
    全部を使ったswapから、抜かれたiでswapされたものだけが入れ替わる形になる（ラベルの張り替えのイメージ）
１つずつ計算すると難しいので、全体で計算したものから軽量に計算できないかを検討する
     */
    ...
    let mut b = (1..=n).collect_vec();
    let mut swaped = vec![];

    for i in 0..m{
        let p = a[i]-1;
        swaped.push((p, b[p], b[p+1]));
        b.swap(p+1, p);
    }

    // #[cfg(debug_assertions)]{eprintln!("{:?}", );}

    let mut poss = vec![0; n+1];
    for i in 0..n{
        poss[b[i]]=i;
    }
    // eprintln!("{:?}", poss);
