...
fn main(){
    ...
    for h in 0..9{
        for w in 0..9{
            if s[h][w] == '#'{vets.push((h as i64, w as i64))}
        }
    }
    ...
    for i in 0..vets.len(){
        for j in i+1..vets.len(){
            for k in j+1..vets.len(){
                for l in k+1..vets.len(){
                    let pts = [vets[i], vets[j], vets[k], vets[l], ];
                    let mut ls = vec![];
                    for i in 0..4{
                        for j in i+1..4{
                            let (y1, x1) = pts[i]; let (y2, x2) = pts[j];
                            ls.push((y1-y2)*(y1-y2) + (x1-x2)*(x1-x2));
                        }
                    }
                    ls.sort();
                    let l=ls[0];
                    if ls[0]==l &&  ls[1]==l &&  ls[2]==l &&  ls[3]==l &&  ls[4]==2*l &&   ls[5]==2*l {res+=1;}
                }
            }
        }
    }
