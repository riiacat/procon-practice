...
use std::io::*;
use std::mem::swap;
use std::ops::{BitAnd, Range, ShrAssign, Neg, Index};
use std::str::FromStr;
use std::sync::Mutex;
use superslice::*;
use ascii::AsciiChar;
...
// #[fastout]
fn main() {
    input![n: usize, m:usize, p: [usize; n], xy: [(usize, usize); m]];
    //new type
    ...
    let mut nodes = vec![];
    for i in 1..=n{
        let n = g.add_node((i, p[i-1]));
        nodes.push(n);
    }

    for (x, y) in xy {
        g.add_edge(nodes[x-1], nodes[y-1], 1);
    }

    let mut loops = petgraph::algo::kosaraju_scc(&g);
    ...
    for (idx,l) in loops.iter().enumerate(){
        ...
        for n_idx in l.iter(){
            let (i, mark) = g.node_weight(*n_idx).unwrap();
                s.insert(i);
                ind[i-1] = idx;
        }
