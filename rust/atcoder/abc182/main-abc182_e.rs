...
// #[fastout]
fn main(){
    /*キー、4方向それぞれから累積的に考える。ブロックがあるところでリセット */
    ...
    input![h:usize, w:usize, n: usize, m:usize, ab: [(usize, usize); n],cd: [(usize, usize); m], ];

    let mut map = vec![vec![0_i64; w]; h];
    ...
    let mut map0 = vec![vec![0_i64; w]; h];
    for h in 0..h{
        for w in 0..w{
            if w == 0{
                map0[h][w] = if map[h][w] == -1{0}else{map[h][w]};
            }else{
                map0[h][w] = if map[h][w] == -1{0}else{map[h][w] + map0[h][w-1]}
            }
        }
    }

    let mut map1 = vec![vec![0_i64; w]; h];
    for h in 0..h{
        for ww in (0..w).rev(){
            if ww == w-1{
                map1[h][ww] = if map[h][ww] == -1{0}else{map[h][ww]};
            }else{
                map1[h][ww] = if map[h][ww] == -1{0}else{map[h][ww] + map1[h][ww+1]}
            }
        }
    }

    let mut map2 = vec![vec![0_i64; w]; h];
    for h in 0..h{
        for w in 0..w{
            if h == 0{
                map2[h][w] = if map[h][w] == -1{0}else{map[h][w]};
            }else{
                map2[h][w] = if map[h][w] == -1{0}else{map[h][w] + map2[h-1][w]}
            }
        }
    }

    let mut map3 = vec![vec![0_i64; w]; h];
    for hh in (0..h).rev(){
        for w in 0..w{
            if hh == h-1{
                map3[hh][w] = if map[hh][w] == -1{0}else{map[hh][w]};
            }else{
                map3[hh][w] = if map[hh][w] == -1{0}else{map[hh][w] + map3[hh+1][w]}
            }
        }
    }
    ...
    for h in 0..h{
        for w in 0..w{
            cnt += if maps.iter().any(|m| m[h][w]>0){1}else{0}
        }
    }
