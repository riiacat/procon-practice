...
// #[fastout]
fn main(){
    /*KEY dpするときに、母数を計算して引くことで計算量を削減することができる場合がある
     */
    ...
    let mut dp = vec![vec![0_usize; n]; k+1];

    let mut noadjs = vec![vec![]; n];
    for &(u, v) in uv.iter(){
        noadjs[u-1].push(v-1);
        noadjs[v-1].push(u-1);
    }
    ...
    for k in 1..=k{
        ...
        for i in 0..n{
            let mut tmp =sm;
            for &noto in noadjs[i].iter(){
                tmp = tmp + MOD - dp[k-1][noto];
                tmp %= MOD;
            }
            tmp = tmp + MOD - dp[k-1][i];
            tmp %= MOD;
            dp[k][i] = tmp;
        }
    }
