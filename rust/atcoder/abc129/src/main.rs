...
fn main() {
    ...
    let mut dp1 = vec![0; l.len()];
    let mut dp2 = vec![0; l.len()];
    ...
    for i in 1..l.len(){
        if l[i] == '0'{
            dp1[i] = dp1[i-1] * 3; dp1[i] %= MOD;
            dp2[i] = dp2[i-1]; dp2[i] %= MOD;
        }else{
            dp1[i] = dp1[i-1] * 3 + dp2[i-1]; dp1[i] %= MOD;
            dp2[i] = dp2[i-1] * 2; dp2[i] %= MOD;
        }
    }

    println!("{}", (dp1[l.len()-1] + dp2[l.len()-1]) %MOD);
}
