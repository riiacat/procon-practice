...
fn main() {
    ...
    let mut cols: Vec<Vec<i64>> = vec![Vec::new(); w];
    ...
    for hh in 0..h {
        ...
        for ww in 0..w {
            if let Some(old) = old {
                if old == s[hh][ww] {
                    ...
                    // rows[hh].push((old, len_col));
                    // rows[hh].push((old, ww - 1));
                    rows[hh].push(ww as i64 - 1);
                    len_col = 1;
                }
            } else {
                len_col += 1;
        ...
        // rows[hh].push((old.unwrap(), w - 1));
        rows[hh].push(w as i64 - 1);
    }
    // eprintln!("{:?}", rows);
    ...
    for ww in 0..w {
        ...
        for hh in 0..h {
            if let Some(old) = old {
                if old == s[hh][ww] {
                    ...
                    // cols[ww].push((old, len_row));
                    // cols[ww].push((old, hh - 1));
                    cols[ww].push(hh as i64 - 1);
                    len_row = 1;
                }
            } else {
                len_row += 1;
        ...
        // cols[ww].push((old.unwrap(), h - 1));
        cols[ww].push(h as i64 - 1);
    }
    // eprintln!("{:?}", rows);
    ...
    for hh in 0..h {
        for ww in 0..w {
            ...
            //tate
            let tate = cols[ww].lower_bound(&(hh as i64));
            let tate = (cols[ww][tate] as i64 + (if tate == 0 { 1 } else { -cols[ww][tate - 1] }));

            //yoko
            let yoko = rows[hh].lower_bound(&(ww as i64));
            let yoko = rows[hh][yoko] as i64 + (if yoko == 0 { 1 } else { -rows[hh][yoko - 1] });

            chmax!(ans, tate + yoko - 1);
            // eprintln!("{}, {}, {}, {}, {}", hh, ww, tate, yoko, ans);
        }
    }
