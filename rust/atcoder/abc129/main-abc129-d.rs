...
fn main() {
    ...
    //4方向に伸ばして、障害物のあった最後のindexを保っておく
    let mut h_last1 = vec![vec![0; w]; h];
    ...
    for w in 0..w{
        ...
        for h in 0..h{
            ...
            h_last1[h][w] = last_block;
        }
    }

    let mut h_last2 = vec![vec![0; w]; h];

    for w in 0..w{
        let mut last_block = h as i64;
        for h in (0..h).rev(){
            if ss[h][w] == '#'{
                last_block = h as i64;
            }
            h_last2[h][w] = last_block;
        }
    }

    let mut w_last1 = vec![vec![0; w]; h];
    ...
    for h in 0..h{
        ...
        for w in 0..w{
            ...
            w_last1[h][w] = last_block;
        }
    }

    let mut w_last2 = vec![vec![0; w]; h];

    for h in 0..h{
        let mut last_block = w as i64;
        for w in (0..w).rev(){
            if ss[h][w] == '#'{
                last_block = w as i64;
            }
            w_last2[h][w] = last_block;
        }
    }
    ...
    for w in 0..w{
        for h in 0..h{
            ...
            let mut res = 0;
            res += h as i64 - h_last1[h][w] ;
            res += h_last2[h][w] - h as i64;
            res += w as i64 - w_last1[h][w];
            res += w_last2[h][w] - w as i64;
            res -= 3;
            chmax!(a, res);
        }
    }
