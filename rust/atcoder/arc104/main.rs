...
use num_traits::{one, zero, Num, NumAssignOps, NumOps, One, Pow, Zero, ToPrimitive};
...
use std::io::*;
use std::mem::swap;
use std::ops::{BitAnd, Range, ShrAssign, Neg, Sub};
use std::str::FromStr;
use std::sync::Mutex;
use superslice::*;
use ascii::AsciiChar;
...
// #[fastout]
fn main() {
    input![n: usize, mut ab: [(isize, isize); n] ];
    //new type

    // ab.sort_by(|(a, b), (a2,b2)|
    //     a.cmp(a2).reverse().then(b.cmp(b2))
    // );
    //
    // eprintln!("{}")
    let ab = ab.iter().map(|a| (a.0 - 1, a.1- 1)).collect_vec();

    let decided = ab.iter().filter(|(a, b)| *a >= 0 && *b >= 0).
        map(|(a,b)| (*a as usize, *b as usize)).collect_vec();

    let mut is_filled = vec![vec![false; 2*n]; 2*n];
    for i in 0..2 * n{
        for j in 0..=i{
            is_filled[i][j] = true;
        }
    ...
    let  ass = ab.iter().filter(|a| a.0 >= 0 && a.1 < 0).map(|a| a.0 as usize).collect_vec();
    let  bs = ab.iter().filter(|a| a.0 < 0 && a.1 >= 0).map(|a| a.1 as usize).collect_vec();
    // println!("{}", ans);

    let  non_count = ab.iter().filter(|a| a.0 < 0 && a.1 <0).count();

    eprintln!("{}, {}, {}", decided.len(), ass.len(), non_count);
    assert_eq!(decided.len() + ass.len() + bs.len() + non_count, ab.len());
    ...
    loop {
        if idx_a == ass.len() && idx_b == bs.len(){
            break;
        }

        // eprintln!("{}, {}", idx_a, idx_b);
        // eprintln!("{}, {}", !(idx_a == ass.len()) , (idx_b == bs.len() || ass[idx_a] < bs[idx_b]));

        if !(idx_a == ass.len()) && (idx_b == bs.len() || ass[idx_a] < bs[idx_b]) {
            //a
            let a = ass[idx_a] as usize;
            for i in a as usize+1 ..2*n{
                if !is_filled[a][i]{
                    is_filled[a][i] = true;
            ...
            //b
            let b = bs[idx_b] as usize;
            for i in (0..b as usize).rev() {
                if !is_filled[i][b] {
                    is_filled[i][b] = true;
                ...
                loop{
                    ...
                    is_filled[i + i_c][ b + i_c] = true;
                    i_c += 1;

                    if i + i_c > 2 * n || b + i_c > 2 * n{
                        break;
                    }
                }
    ...
    if is_filled.iter().map(|v| v.iter().filter(|b| !**b).count()).sum::<usize>() >= non_count{
        println!("Yes");
    }else{
        println!("No");
    }
