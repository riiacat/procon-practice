...
fn main(){
    ...
    let mut res = 0;
    let mut res = cnts[0].len()*cnts[1].len()*cnts[2].len();
    ...
    //         // let idx: usize;
    //         let delta: usize;
    //         if ![s[i], s[j]].contains(&'R'){
    //             let idx = cnts[0].lower_bound(&j);
    //             delta = cnts[0].len() -idx;
    //         }else if ![s[i], s[j]].contains(&'G'){
    //             let idx = cnts[1].lower_bound(&j);
    //             delta = cnts[1].len() -idx;
    //         }else if ![s[i], s[j]].contains(&'B'){
    //             let idx = cnts[2].lower_bound(&j);
    //             delta = cnts[2].len() -idx;
    //         }else{
    //             panic!();
    //         }

    //         res += delta;
    //         #[cfg(debug_assertions)]{eprintln!("{:?}", (i, j,delta));}
    //     }
    // }


    #[cfg(debug_assertions)]{eprintln!("initres: {:?}", (res, (cnts[0].len(),cnts[1].len(),cnts[2].len())));}
