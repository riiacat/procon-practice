...
fn main(){
    ...
    input![n: usize, m: usize, uv: [(usize, usize); m], k: usize, pd: [(usize, usize); k]];
    ...
    let mut g: UnGraph<i64, ()> = petgraph::Graph::new_undirected();
    ...
    for (p, d) in pd {
        // black_cnts[p - 1] += 1;
        // black_idx[p - 1] = d as i32;

        let res = dijkstra(&g, nodes[p-1], None, |n| 1_usize);

        for (n, mind) in res.iter() {
            if *mind < d{
                black_list.insert(n.index());
            }
        }
        let res = res.iter().filter(|(n, mind)| **mind == d).map(|(n, d)| n.index()).collect_vec();
        ress.push(res);
    }
