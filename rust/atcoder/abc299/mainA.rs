...
fn main(){
    ...
    let pos1 = s.iter().enumerate().filter(|&(_, &x)| x == '|').map(|(i, _)| i).collect::<Vec<usize>>();
    let pos2 = s.iter().enumerate().filter(|&(_, &x)| x == '*').map(|(i, _)| i).collect::<Vec<usize>>();

    if pos1[0] < pos2[0] && pos1[1] > pos2[0] {
        println!("in");
    } else {
        println!("out");
    }
