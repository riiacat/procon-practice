...
fn main(){
    ...
    let exist_t = c.iter().filter(|&&x| x == t).count() > 0;

    // eprintln!("exist_t: {}", exist_t);
    let t = if exist_t {
        t
    }else{
        c[0]
    ...
    let t_cards = r.iter().enumerate().zip(c.iter()).filter(|(_, &x)| x == t).collect::<Vec<_>>();
    // eprintln!("t_cards: {:?}", t_cards);
    let max_pos = t_cards.iter().max_by_key(|(a, _)| a.1).unwrap().0;
    println!("{}", max_pos.0 + 1);
