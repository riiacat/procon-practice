...
fn main() {
    ...
    input![mut ab: [(usize, usize); n] ];
    input![mut cd: [(usize, usize); m] ];

    let a = ab.iter().map(|a| a.0).collect_vec();
    let b = ab.iter().map(|a| a.1 + a.0).collect_vec();
    let c = cd.iter().map(|a| a.0).collect_vec();
    let d = cd.iter().map(|a| a.1 + a.0).collect_vec();

    let mut left = 0.0; let mut right=1.0;

    let f = |x|{
        let mut a = a.iter().zip(b.iter()).map(
            |(a, b)| {
                *a as f64 - *b as f64 * x
        }).collect_vec();
        a.sort_by(|a: &f64, b| a.partial_cmp(b).unwrap());
        let mut b = c.iter().zip(d.iter()).map(
            |(c, d)| {
                 *d as f64 * x - *c as f64
        }).collect_vec();
        b.sort_by(|a: &f64, b| a.partial_cmp(b).unwrap());
