...
use std::io::*;
use std::mem::swap;
use std::ops::{BitAnd, Neg, Range, ShrAssign, Index};
use std::str::FromStr;
use std::sync::Mutex;
use superslice::*;
...
fn main() {
    ...
    input![l: usize, n1:usize, n2:usize, ];
    input![vl1: [(usize, usize); n1], vl2: [(usize, usize); n2] ];

    let v1 = vl1.iter().map(|(v, _)| *v).collect_vec();
    let v2s = vl2.iter().map(|(v, _)| *v).collect_vec();

    let l1 = vl1.iter().map(|(_, v)| *v).collect_vec();
    let l2s = vl2.iter().map(|(_, v)| *v).collect_vec();

    let l1_comsum: Vec<usize> = l1.iter().cumsum().collect_vec();
    let l2_comsum: Vec<usize> = l2s.iter().cumsum().collect_vec();

    let mut l2pos = 0;

    let mut res = 0_usize;
    for ((v, l), lc) in v1.iter().zip(l1).zip(l1_comsum){
        let l_s = lc-l;

        let mut lc2 = l2_comsum[l2pos];
        let mut l2 = l2s[l2pos];
        let mut lc2_s = if l2pos > 0{l2_comsum[l2pos-1]}else{0};
        let mut v2  = v2s[l2pos];

        // eprintln!("1:{:?}", (v, l, lc, l_s));
        // eprintln!("2:{:?}", (lc2, l2, lc2_s, v2));
        while lc2 <= lc{
            let delta = if *v == v2{
                if lc2_s <= l_s{
                    //partial
            ...
            // eprintln!("{}, l2pos={}", delta, l2pos);

            l2pos+=1;
            if l2pos == n2{break;}
            lc2 = l2_comsum[l2pos];
            l2 = l2s[l2pos];
            lc2_s = if l2pos > 0{l2_comsum[l2pos-1]}else{0};
            v2  = v2s[l2pos];
        }

        if l2pos < n2 && *v == v2{
            let mut delta = lc - lc2_s;
            chmin!(delta, l);
            // eprintln!("{}, l2pos={}", delta, l2pos);
            res += delta;
        }
    ...
    println!("{}", res);
    // println!("A: {:?}", a_positions); // prints "A: [1, 3, 5, 7, 9]"
// println!("B: {:?}", b_positions); // prints "B: [2, 4, 6, 8, 10]"
//     // eprintln!("{:?}", isp);
//     // println!("{}", n-res);
}
