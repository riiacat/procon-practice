...
fn main(){
    ...
    // println!("Hello, world!");
    input![n:usize, f: [[i64; 10]; n], p: [[i64; 11]; n]];
    // let p = p.iter().map(|v| v.iter().copied().cumsum::<i64>().collect_vec()).collect_vec();

    let mut res = -1_000_000_000_000_i64;
    for b in 1..(1<<10){
        let mut r = 0;
        for mise in 0..n{
            let mut c = 0;
            for jikan in 0..10{
                if (b >> jikan) & 1 == 1 && f[mise][jikan]==1{
                    c+=1;
                }
            }
            if c==0 || b==512{
                #[cfg(debug_assertions)]{eprintln!("{:?}", (b, mise, c, p[mise][c as usize]));}
            }
            r += p[mise][c as usize];
        }
