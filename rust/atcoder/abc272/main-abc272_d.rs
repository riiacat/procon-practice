...
fn main(){
    ...
    let mut visited = vec![vec![-1_i64; n]; n];
    let mut q = VecDeque::new();
    visited[0][0] = 0;
    q.push_back(((0,0), 0));

    let mut edge = 0_i64;
    while edge * edge <= m as i64{edge += 1;}

    // let can_move = vec![vec![false; 2*edge+1]; 2*edge+1];
    let mut can_moves = vec![];
    for h in 0..edge{
        for w in 0..edge{
            if h*h + w*w == m as i64{
                can_moves.push((h, w));
    ...
    // eprintln!("edge: {:?}", edge);
    // eprintln!("can_moves: {:?}", can_moves);

    while !q.is_empty(){
        let (p, d) = q.pop_front().unwrap();

        for &(dh, dw) in can_moves.iter(){
            let newh = p.0+dh;let neww = p.1+dw;
            // eprintln!("new_ops: {:?}", (newh, neww));
            if newh <0 || newh>=n as i64 || neww <0 || neww>=n as i64 {continue;}
            let newh = newh as usize;let neww = neww as usize;
            if visited[newh][neww] != -1{continue;}
            visited[newh][neww] = d+1;
            q.push_back(((newh as i64, neww as i64), d+1));
        }
    }
