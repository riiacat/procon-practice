...
fn main() {
    ...
    let s = s.into_iter().rev().collect_vec();
    let mut dp = vec![ vec![0; 13]; s.len()];
    ...
    for i in 0..s.len(){
        ...
        let mut a = vec![];
        if s == '?' {
            a = Vec::from_iter((0..10).into_iter());
        }else{
            a.push(s.to_digit(10).unwrap());
        }

        for a in a.iter(){
            let modulo = (*a as usize * ten)  as usize % 13;

            if i ==  0 {
                dp[i][*a as usize] = 1;
            }else{
                for j in 0..13{
                    dp[i][(j + modulo) % 13] += if i > 0 {dp[i-1][j] % MOD} else{1};
                    dp[i][(j + modulo) % 13] %= MOD;
                }
            }
        }
