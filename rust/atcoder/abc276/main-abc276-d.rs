...
fn main(){
    ...
    for a in a.iter().copied(){
        ...
        for i in 2..a{
            if i*i > a{
                if aa >1{
                    fs.entry(aa).and_modify(|a| *a+=1).or_insert(1_usize);
                }
                break;
            }
            ...
            if aa % i == 0{
                ...
                fs.entry(i).and_modify(|a| *a+=c).or_insert(c);
            }
        }
        factorss.push(fs);
    }

    // for fs in factorss.iter(){
    //     eprintln!("{:?}", fs);
    // }

    let mut fsst = HashSet::new();
    let mut p2s = vec![];
    let mut p3s = vec![];
    ...
    for fs in factorss.iter(){
        ...
        p2s.push(*fs.get(&2).unwrap_or(&0));
        p3s.push(*fs.get(&3).unwrap_or(&0));
    }

    for p in fsst.iter().copied(){
        if p == 2 || p==3{continue;}

        let cnts = factorss.iter().map(|fs| *fs.get(&p).unwrap_or(&0)).collect_vec();
        let a = cnts[0];
        if cnts.iter().any(|c| *c!=a){
            println!("-1");
            return ;
        }
    }
    ...
    let mut res = 0;
    let mn = p2s.iter().min().unwrap();
    res += p2s.iter().map(|cnt| cnt-mn).sum::<usize>();
    let mn = p3s.iter().min().unwrap();
    res += p3s.iter().map(|cnt| cnt-mn).sum::<usize>();
