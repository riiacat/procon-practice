...
fn main(){
    ...
    for i in 0..n-1{
        let mut sorted = true;
        for j in i+1..n-1{
            if !(p[j] < p[j+1]){sorted=false; break;}
        }
        let mut exist_small = false;
        for j in i+1..n{
            if p[i] > p[j]{exist_small=true; break;}
        }

        if sorted && exist_small{res=i}
    }
    ...
    let a = *p.iter().dropping(res).filter(|a| **a < p[res]).max().unwrap();
    r.push(a);
    let mut rest = p.iter().copied().dropping(res).filter(|b| *b!=a).collect_vec();
    rest.sort_by_key(|a| -a);

    r.extend(rest.iter().copied());
    for v in r.iter(){
        print!("{} ", v);
    }
    println!()
