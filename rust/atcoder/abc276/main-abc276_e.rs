...
// #[fastout]
fn main(){
    /*KEY 行って帰ってくるには、隣接点のどれかを２つをスタート、ゴールとすれば十分 */
    ...
    input![h: usize, w: usize,mut c: [Chars; h]];
    ...
    let start = {
        ...
        for h in 0..h{
            for w in 0..w{
                if c[h][w]=='S'{s=(h,w); c[h][w]='#'}
            }
        }
        s
    };

    for &(dh, dw) in dydx.iter(){
        let (hh, ww) = start;
        if hh as i64+dh<0 || hh as i64 +dh>=h as i64 || ww as i64+dw<0 || ww as i64 +dw>=w as i64{continue;}
        if c[(hh as i64+dh) as usize][(ww as i64+dw) as usize] == '#' {continue;}
        sgs.push(((hh as i64+dh) as usize,(ww as i64+dw) as usize))
    }

    for i in 0..sgs.len(){
        for j in i+1..sgs.len(){
            let s = sgs[i]; let g = sgs[j];
            // eprintln!("s, g: {:?}, {:?}", s, g);
            ...
            while !q.is_empty(){
                let (hh, ww) = q.pop_front().unwrap();

                for &(dh, dw) in dydx.iter(){
                    if hh as i64+dh<0 || hh as i64 +dh>=h as i64 || ww as i64+dw<0 || ww as i64 +dw>=w as i64{continue;}
                    if c[(hh as i64+dh) as usize][(ww as i64+dw) as usize] == '#' {continue;}

                    if ((hh as i64+dh) as usize, (ww as i64+dw) as usize) == g{
                        println!("Yes");
                        return ;
                    }
                    c[(hh as i64+dh) as usize][(ww as i64+dw) as usize]='#';
                    q.push_back(((hh as i64+dh) as usize, (ww as i64+dw) as usize));
                }
            }
        }
    }
