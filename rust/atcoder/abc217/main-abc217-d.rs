...
fn main(){
    ...
    input![l: usize, q: usize, cx: [(usize, usize); q]];

    // let mut marks = vec![0];
    let mut marks = BTreeSet::new();
    marks.insert(0); marks.insert(l);

    for (c, x) in cx.iter().copied(){
        if c==1{
            marks.insert(x);
        }else{
            // if marks.len()==0{println!("{}", l); continue;}

            // let p = marks.lower_bound(&x);
            let l = marks.range(..x).next_back().unwrap();
            let r = marks.range(x..).next().unwrap();

            // eprintln!("marks: {:?}", marks);
            // eprintln!("x, l, r: {}, {}, {}", x, l, r);
