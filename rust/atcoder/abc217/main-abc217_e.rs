...
// #[fastout]
fn main(){
    /*KEY 毎回完全にソートしない方法を考える。BinaryHeapに追加指定行く方法でも慣らし計算量的にOKだった */
    ...
    let mut vecs =vec![vec![]; q];
    let mut poss =vec![0; q];
    let mut cur_pos = 0;
    let mut cur_min = 1_000_000_000+1;
    ...
    for i in 0..q{
        ...
        match id {
            ...
            2 => {
                if mins.len()==0{
                    let a = vecs[cur_pos][poss[cur_pos]];
                    println!("{}", a);
                    poss[cur_pos] += 1;
                }else{
                    let (a, p) = mins.range(..(1_000_000_000+1, q+1)).next().copied().unwrap();
                    println!("{}", a);
                    mins.remove(&(a, p));
                    poss[p] += 1;
                    if poss[p] < vecs[p].len(){
                        mins.insert((vecs[p][poss[p]], p));
                    }
                }
                // let a = vq.pop_front().unwrap();
            },
            3 => {
                let mut v = vecs[cur_pos].iter().as_slice()[poss[cur_pos]..].to_vec();
                v.sort();
                vecs[cur_pos] = v;
                poss[cur_pos] = 0;
                if !vecs[cur_pos].is_empty(){
                    mins.insert((vecs[cur_pos][0], cur_pos));
                }
                cur_pos+=1;
            },
            _ => panic!()
