...
fn main(){
    ...
    // println!("Hello, world!");
    input![h: usize, w: usize, ch: usize, cw: usize, dh: usize, dw: usize,
        s: [Chars; h]
    ];
    let (ch, cw) = (ch-1, cw-1);
    let (dh, dw) = (dh-1, dw-1);
    ...
    for i in -2..=2_i64{
        for j in -2..=2_i64{
            if i==0&&j==0{continue;}
            let cost = if abs(i)+abs(j) == 1{0}else{1};
            dydx.push((i, j, cost));
        }
    }
    // #[cfg(debug_assertions)]{eprintln!("dydx: {:?}", dydx);}

    while !q.is_empty(){
        let (hh, ww, c) = q.pop_front().unwrap();
        if res[hh][ww] != -1{continue;}
        res[hh][ww] = c;
        if hh == dh && ww == dw{break;}
        let hh=hh as i64; let ww =ww as i64;

        for &(dy, dx, d) in dydx.iter(){
            let newh = hh+dy; let neww = ww+dx;
            if newh < 0 || newh>=h as i64|| neww<0 || neww>=w as i64{continue;}
            let newh=newh as usize; let neww = neww as usize;
            if s[newh][neww] == '#'{continue;}
            if res[newh][neww] !=-1{continue;}

            if d == 0{
                q.push_front((newh, neww, c));
            }else{
                q.push_back((newh, neww, c+d));
            }
        }
    }
