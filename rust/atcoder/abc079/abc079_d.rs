...
fn main(){
    ...
    // println!("Hello, world!");
    input![h: usize, w: usize, c: [[usize;10]; 10], a: [[i64; w]; h]];

    let mut cnt = vec![0_usize; 10];
    ...
    let mut min_pairs = vec![vec![vec![1_000_000_000_000; 11]; 10]; 10];

    for i in 0..10{
        for j in 0..10{
            min_pairs[i][j][0]=c[i][j];
        }
    }
    ...
    for k in 0..10{
        for i in 0..10{
            for j in 0..10{
                min_pairs[i][j][k+1]=min!(min_pairs[i][j][k], min_pairs[i][k][k]+min_pairs[k][j][k]);
            }
        }
    }
