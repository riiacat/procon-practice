...
// #[fastout]
fn main(){
    /* KEY 超頂点的なものを考える。最初にグラフを列挙しなくても、スタート地点から行けるところをバックトラック的な感じで列挙した方が自然か
     */
    // let mut stdin = LineSource::new(BufReader::new(io::stdin()));
    // macro_rules! input(($($tt:tt)*) => (proconio::input!(from &mut stdin, $($tt)*)));

    // println!("Hello, world!");
    input![m: usize, uv: [(Usize1, Usize1); m], p: [Usize1; 8]];

    let mut pp = vec![-1; 9];
    for (i, &p) in p.iter().enumerate(){
        #[cfg(debug_assertions)]{eprintln!("{:?}", (p, i));}
        pp[p] = i as i32;
    }
    ...
    let init = (0..9).collect_vec();

    let mut raw_adjs = vec![vec![false; 9]; 9];
    ...
    let mut nid2retu = vec![];

    for (i, retu) in init.iter().copied().permutations(init.len()).enumerate(){
        nid2retu.push(retu);
    }

    let retu2nid: HashMap<Vec<i32>, usize> = HashMap::from_iter(nid2retu.iter().enumerate().map(|(i, retu)| (retu.clone(), i)));

    let mut adjs = vec![HashSet::new(); nid2retu.len()];

    for (i, retu) in nid2retu.iter().enumerate(){
        let pos9 = retu.iter().position(|v| *v==8).unwrap();
        for target_v in 0..9{
            if target_v == pos9{continue;}

            if raw_adjs[pos9][target_v]{
                let mut new_retu = retu.clone();
                let tmp = new_retu[pos9];
                new_retu[pos9] = new_retu[target_v];
                new_retu[target_v] = tmp;

                let to_nid = *retu2nid.get(&new_retu).unwrap();

                adjs[i].insert(to_nid);
                adjs[to_nid].insert(i);
            }
        }
    }

    #[cfg(debug_assertions)]{eprintln!("{:?}", p);}
    let start = *retu2nid.get(&p.iter().map(|v| *v as i32).collect_vec()).unwrap();
    let goal = *retu2nid.get(&init).unwrap();

    let mut q = VecDeque::new();
    let mut vis = vec![false; nid2retu.len()];

    q.push_back((start, 0));

    while !q.is_empty(){
        let (nid, d) = q.pop_front().unwrap();
        vis[nid] = true;
