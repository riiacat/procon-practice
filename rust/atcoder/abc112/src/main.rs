...
// #[fastout]
fn main() {
    input![n: usize, mut xyh: [(i64, i64, i64); n]];

    xyh.sort_by_key(|(_, _, h)| Reverse(*h));
    ...
    for cx in 0..=100 {
        for cy in 0..=100 {
            ...
            let (x, y, h) = xyh[0];
            let calc_h = h + abs(cx - x) + abs(cy - y);

            for (x, y, h) in xyh.as_slice()[1..].iter() {
                let hh = max(calc_h - abs(cx - x) - abs(cy - y), 0);
                if hh != *h {
                    // eprintln!("{}, {}, {}, {}, {}, {}, {},", cx, cy, calc_h, x, y, hh, h);
                    is_ok = false;
                    break;
                }
            }
