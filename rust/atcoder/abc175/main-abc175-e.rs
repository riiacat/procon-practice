...
fn main(){
    ...
    for c in 0..C{
        for r in 0..R{
            ...
            let v = map[r][c];
            if r < R-1{
                // eprintln!("r, c, v: {}, {}, {}", r, c, v);
                chmax!(dp[r+1][c][0], max![dp[r][c][0]+v, dp[r][c][1]+v, dp[r][c][2]+v, dp[r][c][3]]);
            }

            if c < C-1{
                if v > 0{
                    for i in 0..3{
                        chmax!(dp[r][c+1][i+1], dp[r][c][i]+v);
                    }
                }
                for i in 0..4{
    ...
    println!("{}", max!(dp[R-1][C-1][0]+map[R-1][C-1], dp[R-1][C-1][1]+map[R-1][C-1], dp[R-1][C-1][2]+map[R-1][C-1], dp[R-1][C-1][3]));
