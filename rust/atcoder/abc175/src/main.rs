...
// #[fastout]
fn main() {
    input![r: usize, c: usize,k: usize, rcv: [(usize, usize, usize); k]];
    //new type

    let mut dp = vec![vec![vec![0; 4]; c]; r];

    let mut rs = vec![vec![0; c]; r];
    for (r,c,v) in rcv{
        rs[r-1][c-1] = v;
    }
        ...
        for j in 0..c {
            for l in 0..4 {
                ...
                for (dx, dy) in dx.iter().zip(dy.iter()) {
                    ...
                    let from_i = from_i as usize;
                    let from_j = from_j as usize;

                    if *dy == -1{
                            chmax!(dp[i as usize][j as usize][l], dp[from_i][from_j][0] + if l > 0 {c}else{0});
                            chmax!(dp[i as usize][j as usize][l], dp[from_i][from_j][1] + if l > 0 {c}else{0});
                        chmax!(dp[i as usize][j as usize][l], dp[from_i][from_j][2] + if l > 0 {c}else{0});
                        chmax!(dp[i as usize][j as usize][l], dp[from_i][from_j][3] + if l > 0 {c}else{0});
                    }else{
                        chmax!(dp[i as usize][j as usize][l], dp[from_i][from_j][l]);
                        if c > 0 {
                            if l > 0 {
                                chmax!(dp[i as usize][j as usize][l], dp[from_i][from_j][l-1] + c);
                            }
                        }
                    }
                }
                ...
                // eprintln!("{}, {}, {}, c:{}, dp: {:?}", i, j, l, c, dp);
            }
        }
    }

    println!("{}", max!(dp[r-1][c-1][0], dp[r-1][c-1][1], dp[r-1][c-1][2],dp[r-1][c-1][3]));
}
