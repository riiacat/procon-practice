...
// #[fastout]
fn main() {
    input![n: usize, k: usize, p: [usize; n], c: [isize; n]];
    let p: Vec<_> = p.iter().map(|a| a - 1).collect();

    let mut cycles = vec![];
    let mut cycle_idx = vec![-1; n];

    while cycle_idx.iter().any(|a| *a==-1) {
        let mut cycle = vec![];

        let mut cycle_sum: isize = 0;
        let start = cycle_idx.iter().find_position(|a| **a == -1).unwrap().0;

        let mut current = start;
        loop {
            cycle_sum += c[current];
            cycle.push(current);
            cycle_idx[current] = cycles.len() as i64;
            if p[current] == start {
                break
            }

            // eprintln!("{},{}, {}", start, current , p[current]);
            current = p[current];
        }
        cycles.push((cycle_sum, cycle));
    }

    //eprintln!("{:?}", cycles);
    let cycle_cumsum =
        cycles.iter().map(|(_, v)| {
            let scores = v.iter().map(|v| c[*v]).collect_vec();
            let mut double = vec![];
            double.extend_from_slice(&scores.as_slice());
            double.extend_from_slice(&scores.as_slice());
            double
            // double.iter().cumsum::<isize>().collect_vec()
        }).collect_vec();
    //eprintln!("{:?}", cycle_cumsum);

    let mut ans = -1_000_000_000 * 100000;
    for ((loop_sum, v), cumsum) in cycles.iter().zip(cycle_cumsum.iter()){
        let mut mid =0;
        let mut rest_len = min!(k, v.len());

        if *loop_sum > 0{
            let (div, modu) = k.div_mod_floor(&v.len());
            mid += loop_sum * div as isize;
            rest_len = modu;
        }

        let mut rest_score = -1_000_000_000 * 100000;
        for i in 0..v.len(){
            let mut rest_mid = 0;
            for n in 0..rest_len{
                rest_mid += cumsum[i+n];
                chmax!(rest_score, rest_mid);
            }
        }

        //eprintln!("{:?}", v);
        //eprintln!("{:?}", v.iter().map(|v| c[*v]).collect_vec());
        //eprintln!("rest_len: {}, loop: {}, rest: {} = {}", rest_len, mid, rest_score, mid + rest_score as isize);

        chmax!(ans, mid + rest_score as isize);
    }
