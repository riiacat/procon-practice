...
// #[fastout]
fn main(){
    /* オーバーラップを考えると大変なので 境界を全探索する（O(n))。
        ある境界より小さい範囲でどれくらい総和を小さくできるかはacuumできるので、それを左右で計算して足し算
     */
    ...
    for i in 0..n{
        mins0.push(min!(mins0[i]+a[i], l*(i as i64+1)));
    }

    let mut mins1 = vec![0];
    for (i, &a) in a.iter().rev().enumerate(){
        mins1.push(min!(mins1[i]+a, r*(i as i64+1)));
    }

    // first r
    println!("{:?}", mins0.iter().zip(mins1.iter().rev()).map(|(a,b)| a+b).min().unwrap());
}
