...
fn main(){
    ...
    let mut dp1 = vec![0_usize; 10];
    let mut dp2 =  vec![0_usize; 10];
    ...
    for i in 0..n-1{
        let (nxt, old) = if i % 2 == 0{
            (&mut dp1, &mut dp2)
        }else{(&mut dp2, &mut dp1)};
        // let old = &mut dp[1-i%2];

        nxt[1] = (old[1]+old[2])%MOD;
        nxt[9] = (old[8]+old[9])%MOD;

        for j in 2..=8{
            nxt[j] = (old[j-1] + old[j] + old[j+1])%MOD;
        }
    }

    let last = if n%2==0{dp1}else{dp2};
    let mut res =0;
    for i in 1..=9{res += last[i]; res%=MOD;}
    println!("{}", res);
