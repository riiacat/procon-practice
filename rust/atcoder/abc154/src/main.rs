...
fn main() {
    ...
    let mut dp0 = vec![vec![0; k+1]; n.len()];
    let mut dp1 = vec![vec![0; k+1]; n.len()];

    // eprintln!("{:?}", n);

    let num = n[0].to_digit(10).unwrap();
    ...
    for i in 1..n.len() {
        ...
        for k in 1..=k{
            ...
            if num != 0 {
                dp0[i][k] += dp1[i-1][k-1] * (num-1) + dp1[i-1][k];
            }

            dp1[i][k] = if num == 0 {dp1[i-1][k]}else{dp1[i-1][k-1]};
        }
    }
    ...
    println!("{}", dp0[n.len()-1][k] + dp1[n.len() - 1][k]);
}
