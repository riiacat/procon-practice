...
// #[fastout]
fn main(){
    /*　KEY 端っこからもとまる。畳み込み的にwaが一定の添え字を見ていく
     */
    ...
    let mut res = vec![];
    res.push(c[n+m+1-1] / a[n]);

    for i in 1..=m{
        #[cfg(debug_assertions)]{eprintln!("i: {:?}", (i, &res));}
        let wa = n+m-i;
        ...
        for j in 0..n{
            // a^j * b^wa-j
            if wa-j > m  {continue;}
            // if wa-j > m || (m-(wa-j)) >= res.len() {continue;}
            #[cfg(debug_assertions)]{eprintln!("{:?}", (m, wa-j));}
            tmp -= a[j]*res[m-(wa-j)];
        }
        res.push(tmp/a[n]);
    }
