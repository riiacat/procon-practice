...
fn main(){
    /* KEY
    クエリ先読み、１つのクエリで最終的に足し込まれる個数について考える
     */
    ...
    input![h: usize, w:usize, c:usize, q:usize, tnc: [(usize, usize, usize); q]];

    let mut cs = HashSet::new();
    let mut rs = HashSet::new();

    let mut res = vec![0_i64; c+1];
    for (t, n, c) in tnc.iter().copied().rev(){
        if t==1{
            if rs.contains(&n){continue;}
            res[c] += (w-cs.len()) as i64;

            rs.insert(n);
        }else{
            if cs.contains(&n){continue;}
            res[c] += (h-rs.len()) as i64;
