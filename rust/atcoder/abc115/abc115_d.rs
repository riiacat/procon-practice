...
fn main(){
    ...
    let mut takasas = vec![1_usize;51];
    let mut paties = vec![1_usize;51];

    for l in 1..=50{
        takasas[l] = 1+takasas[l-1]+1+takasas[l-1]+1;
        paties[l] = paties[l-1]+1+paties[l-1];
    }

    #[cfg(debug_assertions)]{eprintln!("{:?}", takasas);}
    #[cfg(debug_assertions)]{eprintln!("{:?}", paties);}

    println!("{}", solve(n, x, &takasas, &paties));
}

fn solve(l: usize, x: usize, takasas:&Vec<usize>, paties:&Vec<usize>) -> usize{
    let mut res = 0;

    if x <= 1 || l==0{
        #[cfg(debug_assertions)]{eprintln!("{:?}", (l, x));}
        if l==0{return min!(1, x);}
        else{
            return 0;
        }
    }
    ...
    if x <= takasas[l]/2{
        res+=solve(l-1, x-1, takasas, paties);
    }else{
        res+=solve(l-1,x-takasas[l]/2-1, takasas, paties)+paties[l-1]+1;
    }

    #[cfg(debug_assertions)]{eprintln!("{:?}", (l, x, res));}
    res
}
