...
fn main(){
    ...
    let mut ecount = vec![0_usize; n];
    let mut cand = BinaryHeap::new();
    for i in 0..n{
        for to in adjs[i].iter(){
            ecount[*to]+=1;
        }
    }
    for i in 0..n{
        if ecount[i] == 0{cand.push(Reverse(i));}
    }

    let mut sorted = vec![];
    while !cand.is_empty(){
        let Reverse(nid) =cand.pop().unwrap();
        sorted.push(nid);
        for &to in adjs[nid].iter(){
            ecount[to]-=1;
            if ecount[to]==0{
                cand.push(Reverse(to));
            }
        }
    }
