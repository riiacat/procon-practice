...
fn main() {
    ...
    for a in a.iter(){
        let e = cnts.entry(a).or_insert(0);
        *e += 1_usize;
    }

    let mut cnts = cnts.iter().map(|(k,v)|{(**k, *v)}).collect_vec();
    cnts.sort_by_key(|(k, _)|{*k});
    ...
    for i in 0..cnts.len(){
        if cnts[i].1 >= 3{
            while cnts[i].1 < 3{
                cnts[i] = (cnts[i].0, cnts[i].1-2);
            }
        }
    }

    let n_more: usize = cnts.iter().filter(|(k, v)| *v>1).map(|(k,v)| *v-1).sum();
    if n_more % 2 == 0{
        println!("{}", cnts.len());
    }else {
        println!("{}", cnts.len()-1);
