...
fn main(){
    ...
    let mut a = a.iter().dropping(2).copied().collect_vec();
    a.sort();

    #[cfg(debug_assertions)]{eprintln!("{:?}", (a0, a1, &a));}


    let init_cnt = a.iter().filter(|&&a| a0<=a && a<=a1).count();
    if init_cnt >= m{println!("0"); return ;}
    let m = m-init_cnt;

    let a = a.iter().copied().filter(|&a| !(a0<=a && a<=a1)).collect_vec();

    let mut res = 1_000_000_000_000_000_000_i64;

    #[cfg(debug_assertions)]{eprintln!("{:?}", a);}
    for i in 0..a.len()-m+1{
        let lv = a[i];
        let rv = a[i+m-1];

        chmin!(res, max!(a0-lv, 0)+max!(rv-a1, 0));
    }
