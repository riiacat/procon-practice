...
// #[fastout]
fn main() {
    input![n: usize, a: i64, b:i64, c:i64, l: [usize; n]];
    //new type

    println!("{:?}", dfs(&l, 0, 0, (0, 0, 0), (a, b, c)));
}

fn dfs(l: &Vec<usize>, idx: usize, mp: usize, made: (i64, i64, i64), abc: (i64, i64, i64)) -> i64 {
    let (m_a, m_b, m_c) = made;
    let (a, b, c) = abc;
    if idx == l.len() {
        if mp / 10 > l.len() || (m_a == 0 || m_b == 0 || m_c == 0) {
            return INF;
        } else {
            let ans = mp as i64 + abs(m_a - a) + abs(m_b - b) + abs(m_c - c) - 30;
            // eprintln!("mp: {}, made: {:?}, ans: {}", mp, made, ans);
            return ans;
        }
    }

    return min!(
    dfs(l, idx+1, mp + 10, (m_a + l[idx] as i64, m_b               , m_c                ), abc),
    dfs(l, idx+1, mp + 10, (m_a                , m_b+ l[idx] as i64, m_c                ), abc),
    dfs(l, idx+1, mp + 10, (m_a                , m_b               , m_c+ l[idx]  as i64), abc),
    dfs(l, idx+1, mp, (m_a , m_b, m_c), abc),
    );
