...
use num_traits::{one, zero, Num, NumAssignOps, NumOps, One, Pow, Zero, ToPrimitive, abs};
...
fn main() {
    ...
    // let a = n / 2;
    // let ans =0;
    let mut dp = vec![ vec![ vec![-1 as f64; 101]; 101]; 101];
    ...
    println!("{}", go(&mut dp, a,b,c));

}

fn go(dp: &mut Vec<Vec<Vec<f64>>>, a: usize, b: usize ,c: usize)  -> f64{
    // eprintln!("0: a: {}, b: {}, c: {} = {}", a ,b, c, dp[a][b][c]);
    if dp[a][b][c] >= 0 as f64 {
        // eprintln!("1: a: {}, b: {}, c: {} = {}", a ,b, c, dp[a][b][c]);
        return  dp[a][b][c];
    }

    let a1 = go(dp, a+1, b, c);
    let a2 = go(dp, a, b+1, c);
    let a3 = go(dp, a, b, c+1);
    let ans = (a as f64 *(a1+1.0)+b as f64*(a2+1.0)+c as f64*(a3+1.0)) / (a+b+c) as f64;
    dp[a][b][c] = ans;
    // eprintln!("2: a: {}, b: {}, c: {} = {}", a ,b, c, dp[a][b][c]);
    return ans;
}