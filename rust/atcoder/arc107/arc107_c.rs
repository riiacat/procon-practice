...
// #[fastout]
fn main(){
    /* KEY 独立な状態に気づく、連結成分であることに気づく。どの順番でやっても異なるものができそうなことに気づく
     */
    ...
    for i in 0..n{
        for j in 0..n{
            if a[i].iter().zip(a[j].iter()).all(|(a, b)| a+b<=k){
                if !uf0.same(i, j){uf0.unite(i, j)};
            }

            if a.iter().map(|a| a[i]).zip(a.iter().map(|a| a[j])).all(|(a, b)| a+b<=k){
                if !uf1.same(i, j){uf1.unite(i, j)};
            }
        }
    }

    let mut group0 = vec![vec![]; n];
    let mut group1 = vec![vec![]; n];

    for i in 0..n{
        group0[uf0.find(i)].push(i);
        group1[uf1.find(i)].push(i);
    }

    #[cfg(debug_assertions)]{eprintln!("g0: {:?}", group0);}
    #[cfg(debug_assertions)]{eprintln!("g1: {:?}", group1);}

    let mut kaijo = vec![1; 50+1];

    for i in 1..=50{
        kaijo[i] = kaijo[i-1]*i;
        kaijo[i]%=MOD;
    }
    #[cfg(debug_assertions)]{eprintln!("kaijo: {:?}", kaijo);}

    let mut res0=1_usize;
    let mut res1=1_usize;

    for i in 0..n{
            res0 *= kaijo[group0[i].len()];
            res0 %=MOD;
            res1 *= kaijo[group1[i].len()];
            res1 %=MOD;
    }

    println!("{}", (res0*res1)%MOD);
}
