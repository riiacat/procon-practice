...
fn main(){
    ...
    for i in 0..n{
        for j in i+1..n{
            let mut ok = true;
            for w in 0..n{
                if a[i][w] + a[j][w] <= k{continue;}
                ok = false;
            }
            if ok && !uf1.same(i, j) {uf1.unite(i, j)}
        }
    }
    eprintln!("uf1");
    ...
    for i in 0..n{
        for j in i+1..n{
            let mut ok = true;
            for h in 0..n{
                if a[h][i] + a[h][j] <= k{continue;}
                ok = false;
            }
            if ok && !uf2.same(i, j){uf2.unite(i, j)}
        }
    }
    eprintln!("uf2");
    ...
    let mut res = 1;
    for v in vs1.iter().copied(){
        res *= kaijo[v];
        res %= MOD;
    }
    for v in vs2.iter().copied(){
        res *= kaijo[v];
        res %= MOD;
    }
