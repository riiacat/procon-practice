...
fn main(){
    ...
    let mut ab=a.iter().copied().zip(b.iter().copied()).collect_vec();
    ab.sort_by_key(|(a, b)| *a);
    let a = ab.iter().map(|ab| ab.0).collect_vec();
    let b = ab.iter().map(|ab| ab.1).collect_vec();

    // eprintln!("{:?}", a);
    // eprintln!("{:?}", b);

    let mut dp = vec![vec![0_usize; 5001]; n+1];
    let mut dp2 = vec![vec![0_usize; 5001]; n+1];
    ...
    for i in 0..n{
        for j in 0..5001{
            dp[i+1][j] = dp[i][j] + if j>=b[i]{dp[i][j-b[i]]}else{0};
            dp2[i+1][j] = if j>=b[i]{dp[i][j-b[i]]}else{0};
            dp[i+1][j]%=MOD;
            dp2[i+1][j]%=MOD;
        }
    }
    ...
    // for r in 0..=n{
    //     eprintln!("{}: {:?}", r, dp2[r].iter().take(6).collect_vec());
    // }
    ...
    // for r in 0..=n{
    //     eprintln!("c {}: {:?}", r, dp2[r].iter().take(6).collect_vec());
    // }
