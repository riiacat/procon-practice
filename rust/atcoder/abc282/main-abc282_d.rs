...
fn main(){
    ...
    for i in 0..n{
        ...
        while !q.is_empty(){
            let (nid, color)=q.pop_front().unwrap();

            for &to in adjs[nid].iter(){
                if colors[to] == colors[nid]{
                    println!("0"); return ;
                }
                if colors[to] != -1{continue;}
                let new=1-color;
                if new == 0{nodes0.push(to)}else{nodes1.push(to)}
                colors[to] = new;
                q.push_back((to, new));
            }
        }
        ...
        for &i in nodes1.iter(){
            res += nodes0.len() - adjs[i].len();
        }
        res2 += (nodes0.len()+nodes1.len())*(n-nodes0.len()-nodes1.len());
    }
