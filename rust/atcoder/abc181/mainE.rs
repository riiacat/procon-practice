...
fn main() {
    ...
    let mut dels2 = vec![];
    for i in 0..n / 2 {
        dels1.push(h[2 * i + 1] - h[2*i]);
        dels2.push(h[2 * i + 1 + 1] - h[2*i + 1]);
    }

    let mut dels_comsum1 = vec![];
    dels_comsum1.push(0 as usize);
    for i in 0..n /2 {
        dels_comsum1.push(dels_comsum1.last().unwrap() + dels1[i]);
    }

    let mut dels_comsum2 = vec![];
    dels_comsum2.push(0 as usize);
    for i in 0..n / 2 {
        dels_comsum2.push(dels_comsum2.last().unwrap() + dels2[i]);
    }


    let mut ans = usize::max_value() / 10;
    // eprintln!("{:?}", h);
    // eprintln!("{:?}",w);
    //
    // eprintln!("del1");
    // eprintln!("{:?}", dels1);
    // eprintln!("{:?}", dels_comsum1);
    //
    // eprintln!("del1");
    // eprintln!("{:?}", dels2);
    // eprintln!("{:?}", dels_comsum2);


    for w in w {
        let p = h.lower_bound(&w);
        ...
        if p < n {
            ...
            //left
            mid += dels_comsum1[p/2];
            // eprintln!("mid1: {}", dels_comsum1[p/2]);

            //mid
            mid += if p % 2 == 1  {w - h[p - 1]} else {h[p] - w};
            // eprintln!("mid2: {}", if p % 2 == 1 {w - h[p - 1]} else {h[p] - w});

            //right
            // if p % 2 == 1{
            //     mid += dels_comsum1[n /2 ] - dels_comsum1[p / 2 + 1];
            //     eprintln!("mid3: {}", dels_comsum1[n /2 ] - dels_comsum1[p / 2 + 1])
            // }else{
                mid += dels_comsum2[n / 2] - dels_comsum2[p / 2];
                // eprintln!("mid4: {}", dels_comsum2[n / 2] - dels_comsum2[p / 2]);
            // }
        }else{
            //left
            mid += dels_comsum1[p/2];
