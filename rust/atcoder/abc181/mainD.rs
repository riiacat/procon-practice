...
fn main() {
    ...
    let mut m = vec![0 as isize; 10];

    for s in s.iter() {
        let s = s.to_digit(10).unwrap() as usize;
        m[s-1] += 1;
    }

    let slen = s.len();
    let maxi =
        if slen == 1 { 1 + 1} else if
            slen == 2 {12 + 1} else {125};
    let mini =
        if slen == 1 {1} else if
        slen == 2 {2} else {13};
    ...
    for i in mini..maxi{
        ...
        let t = 8 * i;
        // eprintln!("8*i: {}", t);
        // let d = (t as f64).log10() as i32 + 1;

        let t = AsciiString::from_str(&t.to_string()[..]).unwrap();

        let mut is_cont = true;
        for td in t.chars() {
            // eprintln!("{}", td);
            let td = td.as_char().to_digit(10).unwrap() as usize;
