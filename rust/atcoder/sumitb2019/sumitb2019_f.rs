...
fn main(){
    ...
    // println!("Hello, world!");
    input![t1: i64, t2: i64, a1: i64, a2: i64, b1: i64, b2:i64];
    ...
    #[cfg(debug_assertions)]{eprintln!("{:?}", (a, b));}

    if a*t1 + b*t2>0{
        println!("0", ); return ;
    }
    if a*t1 + b*t2 == 0{
        println!("infinity", ); return ;
    }

    let mut stock = a*t1;
    let delta_by2 = abs(a*t1+b*t2);

    #[cfg(debug_assertions)]{eprintln!("stock, delta_by2: {:?}", (stock, delta_by2));}

    let mut res = 2*(stock / delta_by2) + if stock % delta_by2 == 0{0}else{1};
    println!("{}", res);
