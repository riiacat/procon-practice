...
// #[fastout]
fn main(){
    /* KEY全体のGCDはGCDを何回もやればいい（ので今回は冗長）
        pairwiseについては、素因数に共通部分があった時点でNG
     */
    ...
    let mut min_factor = vec![-1_i64; 1000000+1];
    ...
    // for p in 2..=20{
        ...
        for _ in 0..=1000000{
            ...
            is_prime[q] = false;
            // eprintln!("q, p: {:?}", (q,p));
            if min_factor[q] == -1 {min_factor[q] = p as i64}
        }
    }

    // eprintln!("minfactor: {:?}", min_factor.iter().take(10).collect_vec());
    // eprintln!("isprime: {:?}", is_prime.iter().take(10).collect_vec());

    let mut primes = vec![HashMap::new(); n];

    for i in 0..n{
        let mut a = a[i];

        while (a>1){
            let p = min_factor[a as usize];
            let mut exp=0_usize;

            while(min_factor[a as usize] == p){
                a/=p;
                exp+=1;
            }
            primes[i].insert(p, exp);
        }
    }

    // eprintln!("{:?}", primes);
    let mut all_keys = HashMap::new();
    let mut setwise = true;
    let mut pair_wise = true;
    for map in primes.iter(){
        for (k, v) in map.iter(){
            all_keys.entry(*k).and_modify(
                |(c, cnt)| {
                    pair_wise = false;
                    *c+=*v;
                    *cnt+=1;
    ...
    for (&k, (c, cnt)) in all_keys.iter(){
        // if primes.iter().filter(|m| *m.get(&k).unwrap_or(&0) > 0).count() == n{
            if *cnt == n{
                setwise =false;
            }
    }
    ...
    // for map in primes.iter(){
    //     if map.iter().any(|(k, v)| *all_keys.get(k).unwrap() != *v){
    //         pair_wise = false;
    //     }
    // }
