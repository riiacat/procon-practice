...
// #[fastout]
fn main(){
    /* KEY 素直な尺取, 先にランレングス表現にすると楽そうだった
     */
    ...
    let runlength =get_runlength_encoding(&s);
    #[cfg(debug_assertions)]{eprintln!("{:?}", runlength);}

    let s =runlength;
    let n = s.len();
    for l in 0..n{
        while r < n && (cnt < k || s[r].0=='1'){
            if s[r].0 == '0' {cnt+=1;}
            restmp+=s[r].1;
            r+=1;
        }
        #[cfg(debug_assertions)]{eprintln!("lr: {}, {}, cnt: {}, restmp: {}", l,r, cnt, restmp);}

        chmax!(res, restmp);
        if s[l].0 == '0' {
            #[cfg(debug_assertions)]{eprintln!("l decrement", );}
            cnt-=1;
        }
        restmp-=s[l].1;
    }
