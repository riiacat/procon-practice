...
// #[fastout]
fn main() {
    input![n: usize, ab:[(isize,isize); n]];
    //new type

    let mut is_eat = vec![false; n];

    let mut amb = BinaryHeap::from_iter(ab.iter().enumerate().map(|(idx, (a,b))| ((a+b, idx, a))));
    let mut bma = BinaryHeap::from_iter(ab.iter().enumerate().map(|(idx, (a,b))| ((b+a, idx, b))));
    ...
    loop {
        ...
        if i % 2 == 0{
            loop {
                let ((s, idx, a)) = amb.pop().unwrap();
                // let Reverse((s, idx, a)) = amb.pop().unwrap();
