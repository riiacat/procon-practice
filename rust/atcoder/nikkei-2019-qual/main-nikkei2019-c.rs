...
fn main() {
    ...
    let mut pt1 = ab.iter().map(|a| ((a.0 + a.1), a.clone())).collect_vec();
    // let pt2 = ab.iter().map(|a| a.1 - a.0).collect_vec();

    pt1.sort();

    let mut res=0;
    for (i, (v, (a, b))) in pt1.iter().rev().enumerate(){
        if i%2!=0{
            res -= b;
        }else{
            res += a;
    ...
    // let mut dp = vec![vec![100000; n+1]; 2];
