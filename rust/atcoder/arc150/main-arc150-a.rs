...
fn main(){
    ...
    for i in 0..t{
        ...
        let mut cnts = [0,0,0_usize];
        let mut res = 0;
        let mut ecnt = vec![];
        // ecnt.push(0);
        ecnt.push(if s[0] == '?' {1}else{0});
        for i in 1..n{
            ecnt.push(ecnt[i-1]+if s[i] == '?' {1}else{0});
        }
        // eprintln!("ecnt {:?}", ecnt);
        let cnt1 = s.iter().filter(|&&a| a =='1').count();
        ...
        if cnts[1]==0 && cnt1 + cnts[0] == k{
            // eprintln!("ok first: {}", 0);
            ng |= ecnt.last().unwrap() - ecnt[k-1] > 0;
            res+=1
        }
