...
// #[fastout]
fn main(){
    /* KEY nの和、mの和から組み合わせの超頂点？が作れそうなことが察せられる。あとは単純なBFS
     */
    ...
    for _ in 0..t{
        input![n: usize, m: usize, c: [usize; n], uv: [(usize, usize); m]];

        let get_nid = |i: usize, j: usize|{
            i*n+j
        };

        let mut adjs = vec![vec![]; n*n];
        ...
        for ei in 0..m{
            for ej in ei..m{
                ...
                if c[v1-1] != c[v2-1] && c[u1-1] != c[u2-1]{
                    let nid1 = get_nid(u1-1, u2-1);
                    let nid2 = get_nid(v1-1, v2-1);
                    adjs[nid1].push(nid2);
                    adjs[nid2].push(nid1);

                    let nid1 = get_nid(u2-1, u1-1);
                    let nid2 = get_nid(v2-1, v1-1);
                    adjs[nid1].push(nid2);
                    adjs[nid2].push(nid1);
                }

                if c[u1-1]!=c[v2-1] && c[v1-1]!=c[u2-1]{
                    let nid1 = get_nid(v1-1, u2-1);
                    let nid2 = get_nid(u1-1, v2-1);
                    adjs[nid1].push(nid2);
                    adjs[nid2].push(nid1);

                    let nid1 = get_nid(u2-1, v1-1);
                    let nid2 = get_nid(v2-1, u1-1);
                    adjs[nid1].push(nid2);
                    adjs[nid2].push(nid1);
                }
            }
        ...
        for i in 0..n{
            for j in 0..n{
                #[cfg(debug_assertions)]{eprintln!("({i}, {j}): {:?}", adjs[get_nid(i, j)]);}
                #[cfg(debug_assertions)]{eprintln!("({i}, {j})p: {:?}", adjs[get_nid(i, j)].iter().map(|nid| (nid/n, nid%n)).collect_vec());}
            }
        }

        let start = (0, n-1);
        let goal = get_nid(n-1,0);

        let mut vis = vec![false; n*n];
        let mut q = VecDeque::new();

        q.push_back((get_nid(start.0, start.1), 0));
