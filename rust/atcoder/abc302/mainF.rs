...
fn main(){
    ...
    let mut ss = vec![];
    for i in 0..n{
        input![a: usize, s:[usize; a]];
        ss.push(s);
    }

    ss.sort_by_key(|s| s.len());
    #[cfg(debug_assertions)]{eprintln!("{:?}", ss);}

    let mut visited = vec![false; n];

    let mut nodes = vec![HashSet::new(); m+1];
    ...
    let goals: HashSet<usize> = HashSet::from_iter(nodes[1].iter().copied());

    #[cfg(debug_assertions)]{eprintln!("nods: {:?}", nodes);}
    if nodes[1].intersection(&nodes[m]).count()>0{
        println!("0"); return ;
    }
    #[cfg(debug_assertions)]{eprintln!("start!!");}


    let mut res = n*10;
    for &start in nodes[m].clone().iter(){
        if visited[start]{continue;}
        // eprintln!("start: {}", start);

        // let mut q = BinaryHeap::new();
        let mut q = VecDeque::new();
        q.push_back((start, Reverse(0_usize)));
        // visited[start] = true;

        // let mut found = false;
        while !q.is_empty(){
            let (node, Reverse(d)) = q.pop_front().unwrap();
            if visited[node]{continue;}
            visited[node] = true;

            for &a in ss[node].iter(){
                for &to in nodes[a].iter(){
                    if nodes[m].contains(&to){
                        q.push_front((to, Reverse(0)));
                        continue;
                    }
                    if goals.contains(&to){
                        #[cfg(debug_assertions)]{eprintln!("found!!");}
                        chmin!(res, d+1);
                        // found = true;
                        continue;
                    }
                    if visited[to]{continue;}
                    // #[cfg(debug_assertions)]{eprintln!("visited: {}, d: {}", to, d);}
                    q.push_back((to, Reverse(d+1)));
                }
                // if found{break;}
            }
            // if found{break;}
    ...
    println!("{}", if res == n*10{-1}else{res as i64});
}
