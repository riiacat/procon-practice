...
fn search(cur: usize, s: &Vec<Vec<char>>, buf: &mut Vec<Vec<char>>, n:usize, m:usize) -> bool{
    // if buf.len() ==s.len(){f
    //     #[cfg(debug_assertions)]{eprintln!("buf");}
    //     for r in buf.iter(){
    ...
    for perm in s.iter().permutations(s.len()){
        let mut ok = true;
        for i in 0..n-1{
            let mut cnt =0;
            for p in 0..m{
                if perm[i][p] != perm[i+1][p]{
                    cnt +=1;
                }
            }
            if cnt != 1{ok = false;}
