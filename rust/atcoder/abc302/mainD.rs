...
fn main(){
    ...
    // println!("Hello, world!");
    input![mut n:usize, mut m: usize,  d:i64, mut a: [i64; n], mut b: [i64; m]];
    a.sort(); b.sort(); //let a =a;let b = b;

    let mut res = -1;

    let mut r = 0;
    while r < m && abs(a[0] - b[r]) > d && b[r] <= a[0]{
    // while r < m && abs(a[0] - b[r]) > d {
        r += 1;
    }
    if r != m{
        for l in 0..n{
            // while r < m && abs(a[l] - b[r]) <= d {r+=1;}
            #[cfg(debug_assertions)]{eprintln!("lr: {},{}", l, r);}
            while r < m && b[r] <= a[l] + d {r+=1;}

            if r>0 && abs(a[l]-b[r-1]) <= d{
                chmax!(res, a[l] + b[r-1]);
            }
        }
    }
