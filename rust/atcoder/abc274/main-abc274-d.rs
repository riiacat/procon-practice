...
fn main() {
    ...
    input![a: [i64; n]];
    let mut a1 = vec![];let mut a2 = vec![];

    for (i, a) in a.iter().enumerate(){
        if i % 2 == 0{a1.push(*a)}else{a2.push(*a)}
    }

    let mut dp = vec![vec![false; 2*(1000 * 10)]; n];
    let mut offset = 1000 * 10;
    dp[0][(a1[0] + offset) as usize] = true;
    // dp[0][(-a1[0] + offset) as usize] = true;
    ...
    for (i, a) in a1.iter().enumerate(){
        ...
        let a = *a;
        // dp[i] = dp[i-1].clone();
        for j in 0..2*(1000 * 10){
            if j as i64 - a >= 0 && j as i64 - a < 2*(1000 * 10) {
                dp[i][j] = dp[i][j] || dp[i-1][(j as i64 -a)as usize];
            }
            if (j as i64 +a) >= 0 && j as i64 + a < 2*(1000 * 10){
                dp[i][j] = dp[i][j] || dp[i-1][(j as i64 +a) as usize];
            }
    ...
    if !dp[a1.len()-1][(x + offset) as usize]{println!("No"); return;}

    let mut dp = vec![vec![false; 2*(1000 * 10)]; n];
    let mut offset = 1000 * 10;
    dp[0][(a2[0] + offset) as usize] = true;
    dp[0][(-a2[0] + offset) as usize] = true;
    ...
    if !dp[a2.len()-1][(y + offset) as usize]{println!("No"); return;}
