...
fn main(){
    ...
    input![n: usize, m: usize, xy:[(i64, i64); n], pq: [(i64, i64); m]];

    // println!("{}", s.len() - cnt);
    let inf = 1_000_000_000_000_f64;
    let mut dp = vec![vec![inf; n+m+1+1]; (1<<(n+m+1))+1];

    let xys = [(0,0)].iter().copied().chain(xy.iter().copied().chain(pq.iter().copied())).collect_vec();

    // eprintln!("{:?}", xys);

    dp[0][0] = 0.0_f64;

    // eprintln!("len0:{}, len1: {}", dp.len(), dp[0].len());
    ...
    for s in 0..(1<<(n+m+1)){
        for v in 0..xys.len(){
            ...
            for u in 0..xys.len(){
                ...
                let mut speed = 1.0;
                for i in 0..m{
                    // eprintln!("s_bit, mask_bit: {:#08b}, {:#08b}", s, 1<<(n+m+1-i-1));
                    if s & (1<<(n+m+1-i-1)) != 0 {speed*=2.0;}
                }
                // eprintln!("speed: {}", speed);
                let d = d / speed;

                // eprintln!("oldbit: {:#08b}", s);
                // eprintln!("newbit: {:#08b}", s | (1<<v));
                // eprintln!("change dp value from {} to {}", dp[s | (1<<v)][v], dp[s][u]+d);
    ...
    let mut res = inf;
    let mask = (1 << (n+1))-1;
    // eprintln!("mask_bit: {:#08b}", mask);
    for s in 0..(1 << (n+m+1)){
        if (s & mask) == mask{
            eprintln!("dp_bit : {:#08b}, {:.04}", s, dp[s][0]);
            chmin!(res, dp[s][0]);
        }
    }
