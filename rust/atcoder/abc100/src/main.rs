...
// #[fastout]
fn main() {
    input![n: usize, m: usize, xyz:[(i64, i64, i64); n]];
    //new type

    let (mut x,mut y,mut z) = xyz.iter().fold((vec![], vec![], vec![]),
                                              |(mut vx,mut vy,mut vz), (x,y,z)| {
    ...
    let mut ans: i64 = -10_000_000_000 * 1_000_0;
    for i in 0..2{
        for j in 0..2{
            for k in 0..2{
                // x.sort_by_key(|x| if i == 0 {*x}else{-*x});
                // y.sort_by_key(|y| if j == 0 {*y}else{-*y});
                // z.sort_by_key(|z| if k == 0 {*z}else{-*z});

                let mut units = xyz.iter().map(|(x,y,z)| {
                    (if i == 0 {*x}else{-*x}) +
                        if j == 0 {*y}else{-*y} +
                        if k == 0 {*z}else{-*z}
                }).collect_vec();

                units.sort_by_key(|a| -a);

                let mut mid = units.iter().take(m).sum();
