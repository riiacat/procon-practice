...
fn main() {
    ...
    input![xyz: [(i64, i64, i64); n]];

    let mut res = -1;
    for is_x in [true, false].iter(){
        for is_y in [true, false].iter(){
            for is_z in [true, false].iter(){
                let mut xyz = xyz.iter().map(|(x, y ,z)|{
                        (if *is_x{*x}else{-x})+
                        (if *is_y{*y}else{-y})+
                        (if *is_z{*z}else{-z})
                }).collect_vec();
                xyz.sort_by_key(|a| -a);
                chmax!(res, xyz.iter().take(m).sum());
            }
        }
    }
