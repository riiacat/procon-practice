...
fn main(){
    ...
    let inf = 2000000_usize;
    let mut boxes = vec![vec![]; n+1];
    let mut box_i = 0;
    let mut mins = BTreeSet::new();

    let mut res: Vec<i64> = vec![-1; n];
    for (i, &p) in p.iter().enumerate(){
        // eprintln!("{:?}", mins);
        // eprintln!("res: {:?}", res);
        if let Some((nxt, box_i)) = mins.range((p, 0)..).next().copied(){
            mins.remove(&(nxt, box_i));
            // mins.insert((p, box_i));

            if boxes[box_i].len() == k-1{
                res[p-1] = i as i64 + 1;
                for &p in boxes[box_i].iter(){
                    res[p-1] = i as i64 +1;
                }
            }else{
                mins.insert((p, box_i));
