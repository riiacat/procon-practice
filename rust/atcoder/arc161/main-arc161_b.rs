...
fn main(){
    ...
    let mut ans = vec![0_i64; 1001];
    // let mut ans = vec![0_i64; 10001];
    for a in 1..=1000_i64{
    // for a in 1..=10000_i64{
        let mut res = -1;
        for i in 1..=a{
            if i.count_ones() == 3{
                chmax!(res, i);
    ...
    // for n in 1..=10000{
        ...
        for i in (0..62){
            vs.push(if n & (1<<i) != 0{1}else{0})
        }
        vs.reverse();

        let mut res  = -1;

        #[cfg(debug_assertions)]{eprintln!("n: {:?}", vs);}
        let onecnt = vs.iter().sum::<usize>();
        if onecnt >=3{
            let mut a = 0_usize;
            let mut cnt = 0;
            for i in (0..62){
            ...
            let poss = vs.iter().copied().enumerate().filter(|(_, a) | *a==1).collect_vec();
            let right_pos = poss[1].0;
            let left_pos = poss[0].0;
            #[cfg(debug_assertions)]{eprintln!("left, right pos: {:?}", (left_pos, right_pos));}
            if right_pos <= 61 -2 {
                let keta = (62-1-right_pos-1);
                res = (1<<(62-1-left_pos))|(1<<keta)|(1<<(keta-1));
                println!("{}", res);
            }else{
                // 次に流す。
            }
        ...
        if res == -1{
            let (first_one_pos, _) = vs.iter().find_position(|vs| **vs==1).unwrap();
            //ok
            if first_one_pos<=58{
                let keta = (62-1-first_one_pos-1);
                #[cfg(debug_assertions)]{eprintln!("first_one_pos: {}, keta: {keta}", first_one_pos);}
                res =(1<<keta)|(1<<(keta-1))|(1<<(keta-2));
                println!("{}", res);
            }
        }

        // (62-1-first_one_pos-1) >= 2;

        // if res != ans[n]{
        //     panic!(format!("n: {n}, res: {res}, ans: {}", ans[n]));
        // }


        // #[cfg(debug_assertions)]{eprintln!("cnt: {:?}", cnt);}
        // if cnt==3{
        //     println!("{}", a);
        // }else{
        //     println!("{}", -1);
