...
fn main(){
    ...
    let n_big = n/2;
    let peaks = a.iter().dropping(n-n_big).collect_vec();
    let wakis = a.iter().take(n-n_big).collect_vec();

    #[cfg(debug_assertions)]{eprintln!("{:?}", peaks);}
    #[cfg(debug_assertions)]{eprintln!("{:?}", wakis);}

    for i in 0..peaks.len(){
        if wakis[i] < peaks[i] && peaks[i] > wakis[i+1]{continue;}
        println!("No");
        return;
    }
