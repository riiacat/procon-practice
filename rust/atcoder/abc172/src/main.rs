...
fn main() {
    ...
    let a_cumsum: Vec<usize> = a.iter().cumsum().collect();
    let b_cumsum: Vec<usize> = b.iter().cumsum().collect();
    // eprintln!("{:?}", a_cumsum);
    // eprintln!("{:?}", b_cumsum);
    ...
    for i in 0..n {
        let mut ok = -1i64;
        let mut ng = m as i64;
        let rest = k as i64 - a_cumsum[i] as i64;
        if rest < 0 {
            break;
        }
        // eprintln!("i: {}, rest{}", i, rest);

        while abs(ok - ng) > 1 {
            let mid = (ok + ng) / 2;
            if rest >= b_cumsum[mid as usize] as i64 {
                ok = mid;
            } else {
                ng = mid;
            }
        }
        // eprintln!("{}, {}, {}", i, ok, ok + 1 + (i + 1) as i64);
        chmax!(ans, ok + 1 + (i + 1) as i64);
    }

    for i in 0..m {
        let mut ok = -1i64;
        let mut ng = n as i64;
        let rest = k as i64 - b_cumsum[i] as i64;
        if rest < 0 {
            break;
        }
        // eprintln!("i: {}, rest{}", i, rest);
        ...
        while abs(ok - ng) > 1 {
            let mid = (ok + ng) / 2;
            if rest >= a_cumsum[mid as usize] as i64 {
                ok = mid;
            } else {
                ng = mid;
            }
