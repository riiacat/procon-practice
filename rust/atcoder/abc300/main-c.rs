...
fn main(){
    ...
    //     for j in i+1..ps.len(){
    //         for k in j+1..ps.len(){
    //             if ps[i]* ps[i]*ps[j]*ps[k]*ps[k] <= n{
    //                 // eprintln!("i, j, k: {}, {}, {}", i, j, k);
    //                 cnt += 1;
    //             }
    //         }
    //     }
    ...
    for i in 0..ps.len(){
        ...
        for j in i+1..ps.len(){
            if ps[i]*ps[i]*ps[j]*ps[j] >n || ps[i]*ps[i]*ps[j]*ps[j]*ps[j] > n  {break;}

            let mut ok: i64=-1; let mut ng: i64=ps.len() as i64;
            while ng-ok>1{
                let mid = (ok+ng)/2;
                let mid = mid as usize;
                if ps[i]*ps[i]*ps[j]*ps[mid] <= n && ps[i]*ps[i]*ps[j]*ps[mid]*ps[mid] <= n{
                    ok = mid as i64;
                }else{
                    ng = mid as i64;
                }
            }
            // eprintln!("i, j, ok, ng: {}, {}, {}, {}, delta: {}", i, j, ok, ng, (ok+1-j as i64).max(0));
            cnt += (ok-j as i64).max(0);
        }
    }
