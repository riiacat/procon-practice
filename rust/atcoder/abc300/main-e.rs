...
fn main(){
    ...
    let md = 998244353_i64;
    let modiv = |a| modinv(a, md);
    // dp[n] = 1;
    ...
    let s = cnts.iter().sum::<usize>();

    let mut dp = vec![vec![vec![0_i64; 61]; 61]; 61];
    dp[0][0][0] = 1;
    ...
    for i in 0..61{
        for j in 0..61{
            for k in 0..61{
                if i+1 < 61{
                    dp[i+1][j][k] += dp[i][j][k] * modiv(5);
                    dp[i+1][j][k] %= md;
                }
                if j+1 < 61{
                    dp[i][j+1][k] += dp[i][j][k] * modiv(5);
                    dp[i][j+1][k] %= md;
                }
                if i+2 < 61{
                    dp[i+2][j][k] += dp[i][j][k] * modiv(5);
                    dp[i+2][j][k] %= md;
                }
                if k+1 < 61{
                    dp[i][j][k+1] += dp[i][j][k] * modiv(5);
                    dp[i][j][k+1] %= md;
                }
                if i+1 < 61 && j +1< 61{
                    dp[i+1][j+1][k] += dp[i][j][k] * modiv(5);
                    dp[i+1][j+1][k] %= md;
                }
            }
        }
    ...
    println!("{}", dp[cnts[2]][cnts[3]][cnts[5]]);
