...
fn main() {
    ...
    for i in 0..m{
        cnts.entry(a[i]).and_modify(|a| *a+=1).or_insert(1_usize);
    }

    // eprintln!("{:?}", cnts);

    let mut ini = 1000000000;
    for i in 0..=n{
        if !cnts.contains_key(&i){ini = i; break;}
    }

    let mut res = ini;
    let mut mid = ini;
    for i in m..n{
        if a[i-m] == a[i]{continue;}
        let before_cnt0 = *cnts.get(&a[i-m]).unwrap();
        let before_cnt1 = *cnts.get(&a[i]).unwrap_or(&0);
        ...
        if before_cnt1 == mid{
            // while(*cnts.get(&mid).unwrap_or(&0) > 0){
            //     mid += 1;
            // }
        }
        ...
        {
        let before_cnt0 = cnts.get_mut(&a[i-m]).unwrap();
        *before_cnt0 -= 1;
        }
        {
            cnts.entry(a[i]).and_modify(|a| *a+=1).or_insert(1);
        }
    }
