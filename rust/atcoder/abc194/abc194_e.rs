...
// #[fastout]
fn main(){
    /*KEY 範囲を見てスライドしていくやつ、添字がバグりやすいので注意する（具体的なもので見た方がいい。）。
テストケースがシンプルすぎてバグが検知できない可能性がある。
     */
    ...
    let first_m_cnt = {
        ...
        for i in 0..m{
            map.entry(a[i]).and_modify(|a|*a+=1).or_insert(1_usize);
        }
        map
    };
    ...
    while (ng-ok)>1{
        ...
        let mut is_ok = (0..k).all(|i| first_m_cnt.contains_key(&i)); // && !first_m_cnt.contains_key(&k);

        if is_ok {
            let mut map = first_m_cnt.clone();
            for i in 1..n-m+1{
                if a[i-1] == a[i+m-1]{continue;}
                if a[i-1] <k{
                    let a = map.get_mut(&a[i-1]).unwrap();
                    *a-=1;
                    if *a==0{is_ok=false; break;}
                }

                if a[i+m-1] < k{
                    if let Some(a) = map.get_mut(&a[i+m-1]){
                        *a+=1;
                    }else{
                        map.insert(a[i+m-1], 1);
                    }
                }
                // if a[i+m] == k{
                //     is_ok = false; break;
