...
fn main(){
    ...
    let (start_idx, _) = vs.iter().find_position(|a| **a==aa).unwrap();
    #[cfg(debug_assertions)]{eprintln!("{:?}", (start_idx, &vs));}

    let md =vs.len()-start_idx;
    let bc = modpow(b, c, md);
    #[cfg(debug_assertions)]{eprintln!("bc1: {:?}", bc);}
    let bc = (bc + md - start_idx)%md;
    #[cfg(debug_assertions)]{eprintln!("bc2: {:?}", bc);}
    println!("{}", vs[start_idx + bc]);
}
