...
fn main(){
    ...
    input![n: usize, m: usize, mut ab: [(usize, i64); n]];

    // let mut dp = vec![0; m];
    ab.sort_by_key(|(k, _)| *k);

    let mut pq = BinaryHeap::new();
    let mut selecteds = BinaryHeap::new();
    ...
    for d in 1..=m{
        ...
        if selecteds.len() < d && !pq.is_empty(){
            // eprintln!("push!!");
            selecteds.push(pq.pop().unwrap());
            // eprintln!("selected: {:?}", selecteds);
        }
