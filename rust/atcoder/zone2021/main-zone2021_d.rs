...
fn main(){
    ...
    for &s in s.iter(){
        if s=='R'{is_right=!is_right;continue;}
        if is_right{
            if *res.back().unwrap_or(&'B') == s{
                res.pop_back().unwrap();
            }else{
                res.push_back(s);
            }
        }else{
            if *res.front().unwrap_or(&'B') == s{
    ...
    let res = if is_right{res.iter().collect_vec()}else{res.iter().rev().collect_vec()};
    #[cfg(debug_assertions)]{eprintln!("{:?}", res);}
    let mut i =0;
    loop{
        if i >= res.len(){break;}

        if i<res.len()-1 && res[i] == res[i+1]{
            i+=2;continue;
        }
