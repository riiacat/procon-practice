...
fn main() {
    ...
    let mut cur_bc_len: usize = 0;
    let mut cur_a_count: usize = 0;
    let mut old = 'Z';
    let mut ans = 0;
    for i in 0..s.len(){
        if old == 'A' && s[i] == 'C' || old == 'B' && s[i] == 'A' || old == 'C' && s[i] == 'C'|| old == 'B' && s[i] == 'B'{
            cur_a_count = 0; cur_bc_len=0;
        }

        if old == 'B' && s[i] == 'C'{cur_bc_len+=1; ans+=cur_a_count }
        else if s[i] == 'A' {cur_a_count += 1;}

        // eprintln!("old: {}, s: {}, cur_bc_len: {}, cur_a_count: {}, ans: {}", old, s[i], cur_bc_len, cur_a_count, ans);
        old = s[i];
    }
