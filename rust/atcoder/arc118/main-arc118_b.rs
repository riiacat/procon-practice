...
// #[fastout]
fn main(){
    /* KEY にぶたんでも貪欲でも解ける。にぶたんをする場合、結局貪欲に近い方法で解を構成する必要がある。
     */
    // let mut stdin = LineSource::new(BufReader::new(io::stdin()));
    // macro_rules! input(($($tt:tt)*) => (proconio::input!(from &mut stdin, $($tt)*)));

    // println!("Hello, world!");
    input![k: usize, n: i64, m: i64, a: [i64; k]];
    ...
    for (i, &a) in a.iter().enumerate(){
        let t = m*a/n;
        cnt0.push((i, t, abs(t*n-m*a)-abs((t+1)*n-m*a)));
        n_tanjun+=t;
        res.push(t);
    }

    #[cfg(debug_assertions)]{eprintln!("{:?}", cnt0);}
    #[cfg(debug_assertions)]{eprintln!("{:?}", n_tanjun);}
    cnt0.sort_by_key(|(_, _, retoku)| -retoku);

    for &(i, _, _) in cnt0.iter().take((m-n_tanjun) as usize){
        res[i]+=1;
    }
