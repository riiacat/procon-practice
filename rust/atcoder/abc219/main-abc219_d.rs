...
fn main(){
    ...
    let mut dp = vec![vec![vec![1000_usize; 601]; 601]; n+1];
    dp[0][0][0] = 0;
    ...
    for i in 0..n{
        for tako in 0..=300{
            for tai in 0..=300{
                chmin!(dp[i+1][min!(tako+ab[i].0, x)][min!(tai+ab[i].1, y)], dp[i][tako][tai]+1);
                // chmin!(dp[i+1][tako+ab[i].0][tai+ab[i].1], dp[i][tako][tai]+1);
                chmin!(dp[i+1][tako][tai], dp[i][tako][tai]);
                // if tako+ab[i].0 >= x && tai+ab[i].1 >= y{
                //     chmin!(res, dp[i][tako][tai]+1);
                // }
                // if tako >= x && tai >= y{
                //     chmin!(res, dp[i+1][tako+ab[i].0][tai+ab[i].1]);
                // }
            }
        }
    }

    println!("{}", if dp[n][x][y] > 300 {-1}else{dp[n][x][y] as i64});
    // println!("{}", if res > 300 {-1}else{res as i64});
