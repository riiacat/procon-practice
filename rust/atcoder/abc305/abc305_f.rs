...
fn main(){
    ...
    let mut adjs: Vec<HashSet<usize>> = vec![HashSet::new(); n];
    let mut used_adjs: Vec<HashSet<usize>> = vec![HashSet::new(); n];
    let mut vis = vec![false; n];

    let mut cur =0;
    let mut got_edges = 0;

    let mut candss: Vec<BTreeSet<(bool, usize)>> = vec![BTreeSet::new(); n];

    for i in 0..2*n{
        #[cfg(debug_assertions)]{eprintln!("{cur}, {got_edges}, {:?}", adjs);}
        ...
        if !vis[cur]{
            for &to in tos.iter(){
                if !adjs[cur].contains(&(to-1)){
                    adjs[cur].insert(to-1);
                    adjs[to-1].insert(cur);
                    got_edges+=1;
                }
            }
        }
        ...
        if !vis[cur]{
            candss[cur].extend(adjs[cur].iter().copied().map(|to| (vis[to], to)));
        }

        vis[cur] = true;

        // cands.sort_by_key(|nid| vis[*nid]);
        #[cfg(debug_assertions)]{eprintln!("{:?}", candss[cur].iter().collect_vec());}

        let mut found = false;

        for &(_, to) in candss[cur].iter(){
            #[cfg(debug_assertions)]{eprintln!("{} -> {} check", cur+1, to+1);}
            if vis[to]{
                if used_adjs[to].len() == adjs[to].len()
                // if used_adjs[to].len() == adjs[to].len() ||
                // (used_adjs[to].len() == adjs[to].len()-1&& !used_adjs[to].contains(&cur))
                {continue;}
                else{
                    println!("{}", to+1);
                    used_adjs[cur].insert(to);
                    // used_adjs[to].insert(cur);
                    cur = to;
                    found = true;
                    break;
                }
            ...
            if !used_adjs[cur].contains(&to){
                println!("{}", to+1);
                used_adjs[cur].insert(to);
                // used_adjs[to].insert(cur);
                candss[cur].remove(&(vis[to], to));
                candss[cur].insert((true, to));
