...
// #[fastout]
fn main(){
    /* KEY 部分なので　累積の値を取るようにして f(r)-f(l)した方が早かった
     */
    ...
    input![n: usize, a: [usize; n], q: usize, lr: [(usize, usize); q]];
    ...
    #[cfg(debug_assertions)]{eprintln!("{:?}", cums);}

    for &(l,r) in lr.iter(){
        let pl = a.lower_bound(&l);
        let pr = a.lower_bound(&r);

        let mut res = cums[pr] - cums[pl];
        #[cfg(debug_assertions)]{eprintln!("{:?}", (pl, pr, res));}
