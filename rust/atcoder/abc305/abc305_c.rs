...
fn main(){
    ...
    let minh = cands.iter().map(|(h, w)| *h).min().unwrap();
    let minw = cands.iter().map(|(h, w)| *w).min().unwrap();

    let maxh = cands.iter().map(|(h, w)| *h).max().unwrap();
    let maxw = cands.iter().map(|(h, w)| *w).max().unwrap();
