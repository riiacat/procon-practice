...
// #[fastout]
fn main(){
    /* KEY 貪欲でいける点に気づく、どこまで巻き込んでいるかの計算に二部探索を使う。
    累積和でそれ以降いくつ使ったかがわかるので、すでにどれくらい減らされているかがわかる */
    ...
    input![n: usize, d: i64, a: i64,mut  xh: [(i64, i64); n]];
    xh.sort_by_key(|(x, _) |*x); let xh = xh;

    let xs = xh.iter().map(|xh| xh.0).collect_vec();
    let mut cnt = 0_i64;
    let mut memo = vec![0_i64];

    // eprintln!("xh: {:?}", xh);
    for i in 0..n{
        // eprintln!("i:{}, cnt: {}, memo{:?}", i, cnt, memo);
        let x_cur =xs[i];
        let left_edge = x_cur - 2*d;
        let pos = xs.lower_bound(&left_edge); //max: i
        let used_bomb = memo.last().unwrap() - memo[pos];
        let rest_hp = xh[i].1 - used_bomb*a;
        let new_bomb = if rest_hp <= 0 {0} else{rest_hp / a + if rest_hp % a == 0{0}else{1}};
        cnt+=new_bomb;
        memo.push(memo.last().unwrap()+new_bomb);
    }
