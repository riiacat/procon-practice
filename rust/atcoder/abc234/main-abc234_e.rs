...
// #[fastout]
fn main(){
    /* KEY 事前に全列挙しても全然間に合うケース（入力に依存せず全列挙して、最後にフィルタリングするだけ）
     */
    ...
    xd.reverse();
    #[cfg(debug_assertions)]{eprintln!("xd: {:?}", xd);}

    let mut cands = vec![];
    for &keta in [xd.len(), xd.len()+1].iter(){
        for sa in -9..=9_i64{
            for d0 in 1..=9_i64{
                ...
                for _ in 0..keta-1{
                    let new_d=sa+new_cand.last().unwrap();
                    if new_d < 0 || new_d>=10{invalid=true; break;}
                    new_cand.push(new_d)
                }

                if invalid{continue;}
                let mut new_cand_n = 0;
                let mut ten=1;
                for a in new_cand.iter().rev(){
                    new_cand_n+=a*ten;
                    ten*=10;
                }
                cands.push(new_cand_n)
            }
        }
    }

    #[cfg(debug_assertions)]{eprintln!("cands: {:?}", cands);}
    let res =cands.iter().filter(|a| **a>=x).min().unwrap();
    println!("{}", res);
