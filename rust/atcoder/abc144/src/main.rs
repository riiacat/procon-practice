...
fn main() {
    ...
    let mut ok: isize = a.iter().zip(f.iter()).map(|tpl| tpl.0 * tpl.1).max().unwrap();
    let mut ng: isize = -1;
    ...
    while abs(ok-ng) > 1{
        ...
        for i in 0..n{
            ...
            let add_k = (init - mid) / f[i] + if (mid - init) % f[i] == 0 {0}else{1};
            this_k += add_k
        }

        if this_k <= k {
            //eprintln!("##### ok: {}, {}, {}, {}", ok, mid, ng, this_k);
            ok = mid;
        }else{
            //eprintln!("##### ng: {}, {}, {}, {}", ok, mid, ng, this_k);
            ng = mid;
        }
    }
