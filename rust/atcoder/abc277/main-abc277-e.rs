...
fn main(){
    ...
    input![n:usize, m:usize, k:usize, uva: [(usize, usize, usize); m], s: [usize; k]];

    let mut adjs = vec![vec![]; n];

    for (u, v, a) in uva.iter().copied(){
        adjs[u-1].push((v-1, a));
        adjs[v-1].push((u-1, a));
    }

    let mut sv = vec![false; n];
    for s in s.iter().copied(){
        sv[s-1] = true;
    }
    ...
    while !st.is_empty(){
        let (nid, state, d) = st.pop_front().unwrap();
        if nid == n-1{
            println!("{}", d);
            return ;
        }

        for (to, a) in adjs[nid].iter().copied(){
            if a != state{
                if sv[nid]{
                    if !visited[1-state][to]{
                        visited[1-state][to] = true;
                        st.push_back((to, 1-state, d+1));
                    }
                }
            }else{
                if !visited[state][to]{
                    visited[state][to] = true;
                    st.push_back((to, state, d+1));
                }
            }
        }
    }
