...
// #[fastout]
fn main() {
    // input![mut h: usize, mut w: usize, mut ss: [Chars; h]];
    input![abcd: [(i64, i64); 4]];

    let res = contains_point((abcd[0], abcd[2], abcd[3]), abcd[1])
        || contains_point((abcd[0], abcd[1], abcd[2]), abcd[3])
        || contains_point((abcd[1], abcd[2], abcd[3]), abcd[0])
        || contains_point((abcd[0], abcd[1], abcd[3]), abcd[2]);

    println!("{}", if res { "No"}else{"Yes"});
}


fn contains_point(triangle: ((i64, i64), (i64, i64), (i64, i64)), point: (i64, i64)) -> bool {
    let p1 = triangle.0;
    let p2 = triangle.1;
    let p3 = triangle.2;
    let d1 = (point.0 - p3.0) * (p1.1 - p3.1) - (p1.0 - p3.0) * (point.1 - p3.1);
    let d2 = (point.0 - p1.0) * (p2.1 - p1.1) - (p2.0 - p1.0) * (point.1 - p1.1);
    let d3 = (point.0 - p2.0) * (p3.1 - p2.1) - (p3.0 - p2.0) * (point.1 - p2.1);
