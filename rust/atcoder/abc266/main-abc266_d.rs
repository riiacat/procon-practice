...
fn main(){
    ...
    // println!("Hello, world!");
    input![n: usize, mut txa: [(usize, usize, i64); n]];
    let inf = -1_000_000_000_000_000 as i64;
    let mut dp = vec![vec![inf; 5]; 100000+1];

    txa.sort_by_key(|a| a.0);
    ...
    for i in 1..100000+1{
        ...
        // #[cfg(debug_assertions)]{eprintln!("{:?}", dp[i-1]);}
        // if i > 6{break;}

        let (t, x, mut a) = txa[txa_i];
        if i!=t {
            a = 0;
        }
        ...
        for j in 0..5{
            if j>0{
                chmax!(dp[i][j], dp[i-1][j-1]+if j==x {a}else{0});
            }
            chmax!(dp[i][j], dp[i-1][j]+if j==x {a}else{0});
            if j < 5-1{
                chmax!(dp[i][j], dp[i-1][j+1]+if j==x {a}else{0});
            }
        }
    ...
    println!("{}", dp[100000].iter().max().unwrap());
}
