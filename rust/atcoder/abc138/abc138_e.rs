...
// #[fastout]
fn main(){
    /* KEY 事前に列挙しておく。にぶたんでもできたが、
右にあるcharの位置を右からaccumする感じにすると毎回にぶたんする必要もなくなる
     */
    ...
    let mut scnt = vec![vec![]; 26];

    for (i, &s) in s.iter().enumerate(){
        let s = AsciiChar::new(s);
        scnt[alphabet2idx(s)].push(i as i64);
    }
    ...
    for &t in t.iter(){
        let t  = alphabet2idx(AsciiChar::new(t));
        ...
        if cur_i.is_none(){
            ...
            let next = scnt[t].upper_bound(&cur_i.unwrap());
            if next == scnt[t].len(){
                lcnt += 1;
                cur_i=Some(-1);

                let next = scnt[t].upper_bound(&cur_i.unwrap());
                let next = scnt[t][next];
                cur_i = Some(next);
        ...
        #[cfg(debug_assertions)]{eprintln!("{:?}", (idx2alphabet(t), cur_i, lcnt));}
    }


    println!("{}", s.len()*lcnt as usize + cur_i.unwrap() as usize + 1);
    // println!("{} {}", amax, res);
}
