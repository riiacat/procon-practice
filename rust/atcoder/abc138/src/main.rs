...
fn main() {
    ...
    // let mut dp = vec![std::usize::MAX / 2; t.len()];
    // let mut next_pos = vec![vec![None ;23] ; s.len()];
    let mut pos_map: HashMap<char, Vec<i64>> = HashMap::new();

    for (p, s) in s.iter().enumerate(){
        if  let Some(v) = pos_map.get_mut(s){
            v.push(p as i64);
        }else{
            let v = vec![p as i64];
            pos_map.insert(*s, v );
    ...
    for (i, t) in t.iter().enumerate(){
        if let Some(pos_v) = pos_map.get(t){
            let pos = pos_v.upper_bound(&s_pos);
            if pos == pos_v.len(){
                let diff  =s.len() - s_pos as usize - 1;
                // eprintln!("1-> {}: {}, {:?}, {}, {}", t, s_pos, pos_v, pos, diff);
                ans += diff;
                let pos = pos_v.lower_bound(&0);
                ans += pos_v[pos] as usize + 1;
                s_pos = pos_v[pos];
            }else{
                let diff  =pos_v[pos] - s_pos ;
                // eprintln!("2-> {}: {}, {:?}, {}, {}", t, s_pos, pos_v, pos, diff);
                ans += diff as usize;
                s_pos = pos_v[pos];
            }
        }else{
