...
// #[fastout]
fn main(){
    /* KEY Xとの大小で絶対値の外れ方が変わる。
この変わり方はaをソートしておけば変わり目が一意に定まる。
累積和と組み合わせればOK
     */
    ...
    // let xi = x.iter().copied().enumerate().collect_vec();
    ...
    let mut acum = a.iter().cumsum::<i64>().collect_vec();

    for &x in x.iter(){
        let p = a.lower_bound(&x);
        if p == a.len(){
            //all small
            println!("{}", n as i64 *x-acum.last().unwrap());
        }else if p == 0{
            //all big
            println!("{}", acum.last().unwrap()-n as i64*x);
        }else
        {
            // small
            let res1 = p as i64 *x-acum[p-1];
            let res2 = acum.last().unwrap() - acum[p-1] - (n - p) as i64 *x;
            println!("{}", res1+res2);
        }
    }
