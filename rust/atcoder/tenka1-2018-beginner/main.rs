...
#[allow(dead_code)]
const INF: isize = 1_0000_000_000;
...
fn main() {
    ...
    let mut inp: VecDeque<isize> = VecDeque::new();
    inp.extend(a.iter());

    let mut vq: VecDeque<isize> = VecDeque::new();
    let mut b = true;
    for i in 0..n{
        let next = if ((i+1)/2) % 2 == 0{inp.pop_front()}else{inp.pop_back()}.unwrap();
        ...
        if !b {
            ...
            if abs(inp.front().unwrap_or(&INF) - next) > abs(inp.back().unwrap_or(&INF) - next){
                vq.push_front(next);
            }else{vq.push_back(next);}
        }

        b = !b;
    }

    let v = vq.iter().collect_vec();
