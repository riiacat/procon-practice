...
// #[fastout]
fn main(){
    /* KEY 個数をうまく管理する。Mapでもいいが、制約を見ると配列でも良かったことに気づけるとベター
    KEY dfsで情報をうまく限定するのに、入り口と出口でdo, undoする
     */
    // let mut stdin = LineSource::new(BufReader::new(io::stdin()));
    // macro_rules! input(($($tt:tt)*) => (proconio::input!(from &mut stdin, $($tt)*)));

    // println!("Hello, world!");
    input![n: usize, c: [usize; n], ab: [(usize, usize); n-1]];
    ...
    let mut res = vec![];
    dfs(0, 0, &adjs, &c, &mut HashMap::new(), &mut res);
    res.sort();
...
fn dfs(cur: usize, par: usize, adjs: &Vec<Vec<usize>>, c: &Vec<usize>, st: &mut HashMap<usize, usize>, res: &mut Vec<usize>) {


    let cnt = st.get(&c[cur]).copied().unwrap_or(0);
    #[cfg(debug_assertions)]{eprintln!("{:?}", (cur+1, cnt, &st));}
    if cnt == 0{res.push(cur)}
    st.insert(c[cur], cnt+1);

    for &to in adjs[cur].iter(){
        if to == par{continue;}
        dfs(to, cur, adjs, c, st, res);
    }
