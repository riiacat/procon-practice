...
fn main(){
    ...
    // println!("Hello, world!");
    input![
         ha: usize,  wa: usize,  a: [Chars; ha],
         hb: usize,  wb: usize,  b: [Chars; hb],
        hx: usize, wx: usize, x: [Chars; hx],
    ];
    ...
    let (hx, wx, x) = {
        ...
        (maxh-minh+1, maxw-minw+1, newx)
    };

    #[cfg(debug_assertions)]{eprintln!("hx, hw: {:?}", (hx, wx));}
    #[cfg(debug_assertions)]{eprintln!("{:?}", "newx");}
    for row in x.iter(){
        #[cfg(debug_assertions)]{eprintln!("{:?}", row);}
    }
    ...
    #[cfg(debug_assertions)]{eprintln!("xcnt: {:?}", xcnt);}

    for ph in 0..60{
        for pw in 0..60{
            if ph + hb >= 60 + 1{continue;}
            if pw + wb >= 60 + 1{continue;}

            let mut new = vec![vec![false; 60]; 60];
            ...
            // #[cfg(debug_assertions)]{eprintln!("new: {:?}", (ph, pw));}
            // for row in new.iter(){
            //     #[cfg(debug_assertions)]{eprintln!("{:?}", row.iter().map(|b| if *b {'#'}else{'.'}).collect_vec());}
            // }

            if cnt != xcnt{
                #[cfg(debug_assertions)]{eprintln!("cnt != xcnt", );}
                continue;
            }
            ...
            for sh in 0..60{
                ...
                for sw in 0..60{
                    ...
                    if ok {
                        #[cfg(debug_assertions)]{eprintln!("{:?}", (ph, pw, sh, sw));}
                        println!("Yes"); return ;
                    }
                }
            }
    ...
    // mut ha: usize, mut wa: usize, mut a: [Chars; ha],
    // mut hb: usize, mut wb: usize, mut b: [Chars; hb],
...
// fn bfs(cur: usize, depth: usize, adjs: &Vec<Vec<usize>>, vis: &mut Vec<bool>) -> usize{
