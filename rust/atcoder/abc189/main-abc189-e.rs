...
fn main(){
    ...
    input![n:usize, xy: [(i64, i64); n], m: usize];
    ...
    let mut qss = vec![vec![]; m+1];


    for (qid, (a, b)) in ab.iter().copied().enumerate(){
        // let (q, a) = qs[a-1];
        // let (mut x0, mut y0) = xy[b-1];
    ...
    let mut ress = vec![(0,0); q];

    for (i ,(op, a)) in qs.iter().copied().enumerate(){
        for (qid,b) in qss[i].iter().copied(){
            let (mut x0, mut y0) = xy[b-1];
            if t {swap(&mut x0, &mut y0)}
            ress[qid] = (x0*xa + xb, y0*ya+yb);
            // println!("{} {}", );
        }
    ...
    for (qid, b) in qss[m].iter().copied(){
        let (mut x0, mut y0) = xy[b-1];
        if t {swap(&mut x0, &mut y0)}
        ress[qid] = (x0*xa + xb, y0*ya+yb);
    }
