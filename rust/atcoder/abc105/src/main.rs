...
fn main() {
    ...
    let is_n = n < 0;
    let mut n = (if is_n {n * (-2)} else{n}) as usize;

    let need_digit =
        if n == 0 { 1}else{
            ((n as f64).log(4f64) as usize) + 1
        };

    for i in 0..30{
        n += 2usize.pow((1 + 2 * i) as u32);
    }
    eprintln!("{}, {}", n,need_digit);

    let mut ans = Vec::new();
    let mut idx = 0;
    eprintln!("{}", format!("{:b}", n).len());
    eprintln!("{:b}", n);
    loop {
        // eprintln!("{}", n);
        if n <= 1{
            eprintln!("idx: {}, n: {}", idx, n);
            if idx % 2 == 0{
                ans.push(n & 1);
            }else{
                ans.push(1 - (n & 1) );
    ...
    let  ans = &mut ans.as_mut_slice()[if is_n{1}else{0}..];
