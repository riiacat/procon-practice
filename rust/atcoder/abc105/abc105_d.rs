...
// #[fastout]
fn main(){
    /*　KEY modに対して集計する。ここに追う必要はなく、左（もしくは全体と左に）にどれくらいあるかが分かれば組み合わせの数を計算できる
     */
    ...
    let mut cumsummd = vec![0];

    for &a in a.iter(){
        cumsummd.push((cumsummd.last().unwrap()+a)%m);
    }

    let mut mdcnts = HashMap::new();

    for (i, &c) in cumsummd.iter().enumerate(){
        mdcnts.entry(c).and_modify(|v: &mut Vec<usize>| v.push(i)).or_insert(vec![i]);
    }

    #[cfg(debug_assertions)]{eprintln!("mdcnts: {:?}", mdcnts);}
    #[cfg(debug_assertions)]{eprintln!("cumsummd: {:?}", cumsummd);}

    let mut res = 0_usize;
    for l in 1..=n{
        let target = cumsummd[l-1];

        if let Some(v) = mdcnts.get(&target){
            let d =v.len() - v.lower_bound(&l);
            res += d;
            #[cfg(debug_assertions)]{eprintln!("{:?}", (l, target, d));}
        };
    }
