...
// ##########
lazy_static! {
    static ref CUMSUM: Mutex<Vec<Vec<usize>>> = Mutex::default();
...
// #[fastout]
fn main() {
    input![n: usize, c:usize, diff: [[usize; c]; c], ini_c: [[usize; n]; n]];
    //new type

    {
        let mut cumsum = CUMSUM.lock().unwrap();
        *cumsum = vec![vec![0; c]; 3];
        ...
        for i in 0..c {
            for j in 0..c {
                ...
                for k in 0..c {
                    if k == i || k == j { continue; }
                    chmin!(ans, cumsum[0][i as usize] + cumsum[1][j as usize] + cumsum[2][k as usize]);
                }
            }
        }
        println!("{}", ans);
...
fn dfs(seted: (i64,i64,i64), idx: usize, c_n: usize) -> usize{
    let (a,b,c) = seted;
    if idx == c_n {
        if a == -1 || b == -1 || c == -1{
            return std::usize::MAX / 2;
        }else{
            let cumsum = CUMSUM.lock().unwrap();
            let ans = cumsum[0][a as usize] + cumsum[1][b as usize] + cumsum[2][c as usize];
            // eprintln!("{:?}, {}", seted, ans);
            return ans;
        }
    }

    let mut not_inits = Vec::new();
    if a == -1 {
        not_inits.push(0);
    }
    if b == -1 {
    ...
    let fill_min = not_inits.iter().map(|not_init| {
        let mut seted = seted;
        match not_init {
            0 => seted.0 = idx as i64,
             1 => seted.1 = idx as i64,
             2 => seted.2 = idx as i64,
            _ => panic!("fill_min")
        }
        dfs(seted, idx + 1, c_n)
    }).min().unwrap_or(std::usize::MAX / 2);
