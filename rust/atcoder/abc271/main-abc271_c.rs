...
// #[fastout]
fn main(){
    /* ある巻数までと考えると、残るものと売るものを二部探索で分けることができる。残るものと売ってGETしたのでx冊になれば、確実に埋めることができる（隙間を真面目に数える必要なし）
     */
    ...
    // let mut aa = vec![0];
    let mut aa = vec![];
    aa.extend(a.iter().copied());
    // aa.push(1_000_000_001);
    aa.sort();



    let mut nonneed_cnt = 0;
    let mut old = -1;
    let mut aaa = vec![];
    for &aa in aa.iter(){
        if old==aa{nonneed_cnt+=1}
        else{aaa.push(aa)}
        old=aa;
    }
    let a = aaa;
    ...
    // let mut blanks = vec![];
    // for i in 0..(a.len()-1){
    //     blanks.push(a[i+1]-a[i]-1);
    // }
    // let mut cum_blank = blanks.iter().cumsum::<i64>().collect_vec();

    // eprintln!("a: {:?}", a);
    // eprintln!("b: {:?}", blanks);
    // eprintln!("b: {:?}", cum_blank);
    ...
    while abs(ok-ng) > 1{
        let mid = (ok+ng)/2;
        let nokori = a.upper_bound(&mid);
        let uru = (nonneed_cnt+a.len()-nokori)/2;

        if nokori+uru >= mid as usize{ok=mid;}else{ng=mid;}
