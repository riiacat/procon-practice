...
fn main(){
    ...
    input![n: usize, k :usize, to: [i64; n]];

    let to = to.iter().map(|&to| to-1).collect_vec();

    let mut vs = vec![];
    let mut previ = vec![-1_i64; n];
    let mut cur = 0_i64;

    let mut i = 0;
    while previ[cur as usize] == -1{
        vs.push(cur);
        previ[cur as usize] = i;
        cur = to[cur as usize];
        i += 1;
    }
    let start = previ[cur as usize] as usize;
    let d = (i as usize- start);

    // eprintln!("vs: {:?}", vs);
    // eprintln!("previ: {:?}", previ);
    // eprintln!("i: {:?}, start: {}, d: {}", i, start, d);
